...
> **State on 2026-09-19:** built, host-tested and marker-checked. **Never
> launched against the game.** Nothing here is known to fix the startup crash or
> the in-play lag until the [launch gate](#launch-gate) is filled in, and players
...
- **Measured vs assumed.** `fnarray_validation_test.cpp` (118 checks; 16
  ...
  message: it needs Zydis). The fault site is **inferred** from
  the log. Unknown until the first launch: what the wrongly picked global is,
  how many candidates this exe has, whether the real mov is inside the window.
  Assumed: the first 8 builtins all have a routine inside the game image. The
  other stage 2 finders use the same first-match heuristic and were never
  reached by the fresh build, so their behaviour on this game is unknown.
...
- **Measured vs assumed.** `yyerror_ledger_test.cpp` (94 checks) and
  ...
  about two minutes of play on the previous DLL. **Not measured:** what a report
  costs on Hero Siege and whether the reports are what makes the game lag - the
  cost and summary lines exist so the first launch measures it. Known
  limitation: a fatal error whose text was already reported shows no banner and
  gets no fresh stack trace. Inherited: the slot itself is still read unchecked,
  and logging stays as thread-unsafe as upstream.
...
- **Measured vs assumed.** A plain `cl /O2` build of the previously documented
  ...
  effect of the optimisation level has been measured. The exception model
  is stated because `HkYYError`'s frame holds C++ objects that rely on it **if**
  the runner's `YYError` leaves by unwinding; whether it returns or unwinds is
  not verified - 0005's `returned=` / `dropped=` counters are what the first
  launch reads it from.
...
- **Measured vs assumed.** The frame measured on Hero Siege (rsp = rbp - 0x100,
  ...
  source; no launch has produced a refusal, so it is **not observed**.
...
| First launch against the game | checklist below | - | **NOT RUN** | - |
| Lag measurement | 0005's cost and summary lines, plus play | - | **NOT RUN** | - |

No submodule pin should move to a build of this series until the last two rows
carry a date, the game build, the DLL's sha256 and the kept logs.
...
| Patch | The log must show | A failure looks like |
| --- | --- | --- |
| 0006 | First line: `[hs] YYToolkit 4.0.1, Hero Siege patch series hs.1 (hero-siege-offline-toolkit, third_party/yytoolkit)` | No such line: the DLL that ran is not a build of this series. Any item below failing only with 0006 applied: drop 0006 and rebuild. |
...
DLL in the same kind of session. If it still lags with R near D and small report
totals, the reports were not the cause - go to the candidates under
[Known limitations](#known-limitations-and-what-is-not-measured). Write "not
observed" rather than "fixed" until both are in hand.
...
- **Never launched.** Every "fixes" in the patch messages is a design intent
  until the launch gate is filled in.
- **Lag candidates this series does not address and nobody has measured:** the
  console YYToolkit allocates (synchronous console writes if the runner prints
  each error) and plugin-side per-call allocations in `CallBuiltinEx`. The
  report path (0005) and the unoptimised build (0006) are addressed but their
...
- **The evidence files the patch messages cite are not in the repository.**
  ...
  `documented/...` in the `Evidence:` fields name files kept on the researcher's
  machine. They are not committed because they carry build-host and
  user-profile paths, and this directory holds no logs (a test enforces both).
  Each can be re-derived: the first is the `YYToolkit.log` of an unpatched
  v4.0.1 build with the two previously documented files applied, launched
  against the game; the second is a strings listing of the `bb113eef...` DLL;
  the third is the two whole-file copies in the submodules'
  `yytoolkit-modified/` directories. The line numbers in the citations hold
  for those local copies only. The counts taken from them (736 log lines, 723
  of them dump; 202 / 100 / 512 / 25; `rsp+0x60`) are repeated in the patch
  messages and in this guide, so the launch gate can be read without the files.
