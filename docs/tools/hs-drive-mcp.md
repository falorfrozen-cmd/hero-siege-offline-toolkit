...
| `hs_bag_tab` | writes (a sub-tab) | `tab` `materials`\|`socket`, `backup_id` (both required) | `tab`, `selected_before`, `selected_after`, `activeNode_before`, `activeNode_after`, `focus_note`, `verb_trail[]`, `layout_trail[]`, `proof[]`; any other `tab` refuses `route_not_measured` |
...
| `route_not_measured` | A tool whose game route no live session reproduced by name refuses before sending anything (`verb_trail: []`): today `hs_skill_bind`, `hs_talent_reset`, `hs_give_item(to="stash")` and `hs_bag_tab` with any sub-tab but `materials`/`socket`. `hs_stash_tab` also answers it after `stashtab` refused a tab whose handler is none of the shapes live 2 reproduced (nothing called). Shared by every hs-drive tool that writes game state. |
...
| `process_exited` | false | It was there and went away; the detail carries the engine's own `launch_status()` message, which names a startup exit. Only a `not_running` gate reading produces it: if the process snapshot fails mid-wait (`unknown`), the tool refuses `game_state_unknown` with `phase: "readiness"` and the gate's own `game_state` instead, exactly as the pre-wait check does, because a snapshot that failed says nothing about whether the game is still up. |
...
The timeout counts from the `Play` click, not the call's start: navigation
(arming and three screens of listing polls, 13–18 s at the live gate) is
bounded by the listing polls' own budget, so a short `timeout_s` still polls
for the load rather than spending itself before `Play`. A client of any size works if the listing's `window=`
