...
| **AFK FARM** | `HS-AFK-Expedition/` | [HS-AFK-Expedition Instructions](HS-AFK-Expedition/instructions.md) | Not in the hub catalog (2026-09-25) | Timed offline expeditions: calibrated farming pace, native reward delivery through an Aurie plugin, Infinite Vault transfers. |
...
| **HS Drive MCP Server** | `tools/hs_drive_mcp/` *(not a submodule)* | [hs-drive MCP Server](../tools/hs-drive-mcp.md) | Ready | Local stdio MCP server: Hero Siege process/EAC/mod-chain status, instrument self-check, fail-closed `hs2saves\` backup and restore, and driving the modded game — launch through ForgePact's engine, `bp_ipc` command send with reply capture, window screenshot, keyboard/mouse injection into the game's own window (`hs_input`), selecting a character from the title screen into a loaded game (`hs_select_character`), graceful close. One machine-wide game lease (`hs_lease_acquire`, `hs_lease_status`, `hs_lease_release`) so two sessions in different worktrees never drive the one install at once. Developer tooling; nothing ships to a player. |
...
| **Modified YYToolkit** | `third_party/yytoolkit/` *(not a submodule)* | [Guide](../../third_party/yytoolkit/README.md) · [notice](../../third_party/yytoolkit/NOTICE.md) · [ADR 0002](../adr/0002-modified-yytoolkit-is-a-patch-series-in-the-hub.md) | Built, host-tested and **launched twice against the game (2026-09-19: first YYToolkit alone, then with ForgePact's plugin and the Tracker producer both loaded, an idle IPC-smoke-only session)** | The source of truth for the toolkit's modified `YYToolkit.dll`: one pinned upstream commit (`upstream.json`), a documented patch series (`patches/`, order in `patches/series`) and the build tool `tools/build_yytoolkit.py`. AGPL-3.0 aggregate; no binary is committed. ForgePact and HS-Offline-Tracker have each merged the pull request that moves their pin to a build of this series (sha256 `51a393d7…`), and the hub's own gitlinks record those merges; the hub library release that build is drawn from is itself published, but neither tool has cut a release carrying the new pin, so players still receive the earlier DLL until one does. |
...
- The submodules' `yytoolkit-modified/` directories (`ForgePact/yytoolkit-modified/`, `HS-Offline-Tracker/aurie-loader/yytoolkit-modified/`) are **not** the source of truth. On `origin`, each now holds a `NOTICE.md` pointing at this series plus a `YYToolkit-BUILD-INFO.json` — the pull requests that rewrote them, replacing the two whole-file copies each used to carry, have merged. The hub library release that pointer targets is itself published, but neither tool has cut a release with it yet, so players still receive the earlier DLL (SHA-256 `bb113eef…`), whose notice was incomplete: strings in that binary show changes it does not list, and the source for them was not kept. See [`docs/agents/yytoolkit-provenance.md`](../agents/yytoolkit-provenance.md) and the per-module guides above.
