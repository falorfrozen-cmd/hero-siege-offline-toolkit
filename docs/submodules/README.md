...
- The submodules' `yytoolkit-modified/` directories (`ForgePact/yytoolkit-modified/`, `HS-Offline-Tracker/aurie-loader/yytoolkit-modified/`) are **not** the source of truth. On `origin`, they hold two whole-file copies and a `NOTICE.md` for the DLL those tools still distribute (SHA-256 `bb113eef…`), and that notice is incomplete: strings in that binary show changes it does not list, and the source for them was not kept. See [`docs/agents/yytoolkit-provenance.md`](../agents/yytoolkit-provenance.md). Each tool has an open, unmerged pull request that already rewrote its copy to a pointer at this series (`NOTICE.md` plus `YYToolkit-BUILD-INFO.json`, no whole-file copies); the hub library release that pointer targets is itself now published, but neither pull request has merged and neither tool has shipped a release with it — see the per-module guides above.
