...
| **HS Drive MCP Server** | `tools/hs_drive_mcp/` *(not a submodule)* | [hs-drive MCP Server](../tools/hs-drive-mcp.md) | Ready | Local stdio MCP server: Hero Siege process/EAC/mod-chain status, instrument self-check, and fail-closed `hs2saves\` backup and restore. Developer tooling; nothing ships to a player. |
