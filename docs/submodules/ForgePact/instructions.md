...
| `py -m unittest discover -s tests -v` | `ForgePact/` | PowerShell / CMD | Python 3.10+ | Executes all 497 Python contract tests (including the native behavior harnesses, which skip without a C++ toolchain). | Read-only test execution; all tests pass | Verified 2026-09-17 |
...
- **Game Executable Updates:** When a new Season 10 patch releases, verify that spawner object names, GML function names, and `LoadDrops` drop family indices remain valid. The player build contains **no raw game addresses** to re-verify (Known Limitations item 11) — everything resolves by name or off a YYToolkit struct — so a new build should surface as named lookups failing, not as a crash. The dev-only `kCiCallMethodFnRva` does need re-verifying before anyone runs `citrace collect confirm native`; `citrace dispatchdump` / `citrace symdump` in the research build are the tools for re-locating it.
  - **Closure names move with every game patch.** The 2026-09-17 `hs-game-sdk` regeneration (hub `4539e68`) found the game patched (`data.win` `2fc37b1b…` → `07D864C9…`): every `anon@N` closure number shifted, while no plain script name changed. Four closure literals reachable from the shipped build now name closures that do not exist on the current game - `anon@97@gml_Object_Spawn_Chaos_Tower_obj_Create_0` (now `anon@102`), `anon@119@gml_Object_Spawn_Shadow_Realm_obj_Create_0` and `anon@119@gml_Object_Spawn_Abyss_obj_Create_0` (both now `anon@126`), and `GenerateItemHash@anon@4638@s_ItemInstanceStruct@InventoryV2Funcs` (now `anon@4791`) - so those `HookOneScript` / `GetNamedRoutinePointer` calls print not-found and the special-content hooks they arm are silent. This is a **static finding, not live-measured**, tracked as D6 of the prospect window workorder; the recommended fix is to spell those names through `HeroSiege::Scripts` constants (so the next regeneration fails the compile instead of a feature) plus a contract test, over `strip_research_blocks`, that every `anon@N@` literal the player build can reach equals an SDK constant's value. The 17 research-only `citrace` closure literals are stale the same way. `prospectprobe`'s table already follows that rule (Phase 0a was blind for exactly this reason).
