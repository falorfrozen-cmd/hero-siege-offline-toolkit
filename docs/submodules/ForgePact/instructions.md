...
23. **Half of the Relics slider does nothing: `droprate group relic` divides bases `DropRelic` never reads (recorded 2026-09-24, issue #162):**
    - **What is measured.** `DropRelic` does not read `droprate.base`: all 156 relics sat at 25,000,000 and relics still dropped constantly (measured 2026-08-28, ForgePact `c0a6a6b`; not re-measured since). `build_key_cmds` still sends `droprate group relic <m>` with every Relics setting, and the plugin's `relic` branch of `DropRateCmd` still divides all relic bases, so that half of the slider writes values nothing reads. The slider's real effect is the other half: the type-41 gate through `dungeonkey`, thinned by the quadratic pre-roll (`g_DkTipOlcek`).
