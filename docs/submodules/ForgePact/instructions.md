...
| `py -m unittest discover -s tests -v` | `ForgePact/` | PowerShell / CMD | Python 3.10+ | Executes all 636 Python contract tests (including the native behavior harnesses, which skip without a C++ toolchain). | Read-only test execution; all tests pass | Verified 2026-09-18 |
...
14. **The toggle-skill active indicator's ownership and timing are inferred, not proven, on two axes (issue #11, Track B, `toggleborder`):**
