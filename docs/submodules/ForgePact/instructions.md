...
| `py -m unittest discover -s tests -v` | `ForgePact/` | PowerShell / CMD | Python 3.10+ | Executes all 441 Python contract tests (including the three native behavior harnesses, which skip without a C++ toolchain). | Read-only test execution; all tests pass | Verified 2026-09-16 |
...
10. Check that the "ForgePact notes cleanup" run publishing started has
    ...
    (for 1.3.20: 1.3.17, 1.3.18, 1.3.19 and 1.3.20). If it failed, re-run it
    with the tag. See Representative Change Workflow §6.
