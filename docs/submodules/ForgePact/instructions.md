...
- `tests/`: Automated Python contract test suites validating plugin source invariants and panel logic without requiring a running game instance.
  ...
  - `test_minimap_smooth_behavior.py` + `minimap_smooth_harness.cpp`: **Behavioral** suite for the minimap smoothing core (ForgePact issue #19, research stage). Splices the real `plugin/include/ForgePact/MinimapSmoothManager.hpp` in at `// PRODUCTION_MINIMAPSMOOTH` and plays the game's part - a marker container refilled every N frames and drawn every frame. Baseline tests (`test_baseline_*`: toggle off, and a game refreshing every frame, both write nothing) and target tests (`test_target_*`: linear glide, arrival, late-refresh clamp, no pop on an early refresh, new/dropped markers, the 128-marker cap counted, index-only identity). Each target scenario's observed failing line against a deliberately broken core is recorded in the harness. Skips without a C++ toolchain.
  - `test_minimap_smooth_contract.py`: Pins the research stage of minimap smoothing: `mmprobe` is research-build only and absent from `kPlayerCommands`, no `mmsmooth` command reaches the player build yet, the core header declares its four tuning constants and includes nothing from the runtime, the probe's target table names every static-search candidate plus the `CheckPlayerInteraction` control, and the probe refuses an address outside `Hero_Siege.exe` before `MmCreateHook`. Round 1 (the instrument-blindness review) adds: the `objMinimap` event detours log `self` only (an object event gets `self`/`other` and nothing else, so reading `argc`/the argument array would read garbage and end the live session) while script detours still log and forward their arguments (the negative control); a resolved pointer that is itself code is refused before it is read as a `CScript`; the event rows are commented as expected `not found` per session 7; `watch` skips failed reads before comparing and has a `list` mode; `hold` decides the element's kind before any instance check and before either write, prints a read-back, and runs from `MmProbeWatchTick` (never `FrameCallback`, never the ship build). The doc-ordering assertions read `docs/minimap-smoothing-research.md`: in § Live procedure the first `mmprobe vars objMinimap`, `mmprobe watch` and `mmprobe hold` all come before the first `mmprobe hook`, with the `minimapShowMonsters` and `Player_obj x` controls present and the old "stop on equal cadence" shortcut gone; § Deciding the hypothesis sits between the procedure and § Results and requires positive evidence for H2 (`surface_exists`, `hold`) and H0; the doc records session 7 and an `H` row. Round 2 (the round-1 review) adds: `hold`'s decision and write live in `MmProbeHoldApply`, which hands `instance_exists` only the kinds `HhResolveInstance` does (`VALUE_REF` is checked before the builtin appears; a method value is refused, and every other kind is `refused without any runtime call`, since a string or `undefined` inside that builtin is a fatal GML dialog); the first-write line prints `typeof=` and `at=`, and the source no longer claims an instance can never look like a struct; `MmProbeHoldRestore` is called from both the normal end and an abort; and the `at`-form (`hold … at <label> pre|post`) writes from inside the script detour - two `MmProbeHoldApply()` calls around `orig(S, O, R, argc, A)`, a reentrancy guard, never from an event detour, `hook it first` and event rows refused at start, the frame tick only counting down while armed, and nothing reaching the ship build. Two doc tests pin that H2's row requires the `at <R1 row> post` displacement control (with performance mode on) with `surface_exists` supporting only, that H0 requires `R10`, the round-2 sentence that a hold never displaced through any write point proves nothing about the draw, that `at <R1 row> post` comes after the first hook with a `by eye` step before it, and that the doc no longer overstates session 7 (a `V` row and `consistent with session 7` in § Results). The split fix (after the round-2 review) adds three tests: `test_research_doc_h2_and_h0_need_a_displacement_control` is extended so H2's control is now the mode-on `at <R1 row> post` hold only - a mode-off displacement (step 8) ties the record to the mode-off draw only and never satisfies (c') - and every other array, list or struct candidate at the refresh cadence from step 3 must also be held at step 7 (mode on) and excluded (c'', superseded in round 1 below); `test_research_doc_h0_is_contradicted_by_v` pins H0's new (i): a by-eye reading (V) that jumps with performance mode on and moves every frame with it off contradicts H0; `test_hold_at_stops_when_the_minimap_instance_changes` pins that an `at` hold pins the objMinimap instance at Start and stops - restore attempted - on `minimap instance changed`; `test_hold_reentrancy_guard_is_cleared_by_a_scope_guard` pins that the reentrancy flag is now cleared by an `InApplyScope` RAII guard rather than a standalone statement, so an exception inside `MmProbeHoldAbort` cannot leave it stuck. Round 1 (the split-fix review) adds four more tests: `test_research_doc_h2_exclusion_fails_closed` pins that H2's (c'') is now a fail-closed **draw-side store ledger** (R11) with a fixed enumeration (every `objMinimap` variable; every `Player_obj` variable and global whose name contains minimap, marker, radar, icon or blip, via `gnames`/`gjson`) whose entries are `excluded (held)`, `excluded (scalar)` or `not excluded (<reason>)` - anything not measured stays `not excluded`, never excluded by default - and that a concluded H2 quotes the enumeration's limit; `test_research_doc_h0_fails_closed` pins that H0 now needs a displacement with performance mode **on** (step 7 or 12, never the mode-off step 8), a `watch` that actually measured a cadence on both runs (at least two changes, `read failures=0`), and V recorded at step 1 with both answers; `test_hold_start_checks_the_object_before_instance_find` pins that `mmprobe hold ... at` checks the resolved object (`oi.ToDouble() < 0`) before calling `instance_find`, not after; `test_hold_says_depends_on_not_reads` pins that the hold's end line and the header comment say the draw **depends on** the record, field and route rather than **reads** it, since a copy the game makes after the refill row returns would displace too. Round 2 (the split-fix review of round 1) adds three more tests, found by auditing every input a closing verdict consumes rather than patching the two named spots: `test_probe_prints_say_what_failed` pins that `MmProbeVars`/`GlobalNames` print `uncontrolled` on an empty or partial return (a count line alone reads as a complete enumeration) and that `mmprobe hold ... at` separates `unknown object` from `no ... instance to pin` from `not an instance kind`; `test_research_doc_ledger_enumeration_is_controlled` pins that each enumerating command (`mmprobe vars`, `gnames`) is checked in-session against a name already known on that object through a *second* builtin that does not share its route (`oget`/`iget`/`gjson`/`cb instance_number`), and that any control failure makes the whole ledger `not excluded (enumeration uncontrolled: <command>)`; `test_research_doc_scalar_exclusion_refuses_a_possible_handle` pins that the non-negative-integer test is hoisted over both `excluded (scalar)` branches, so any handle-shaped value (changed or not) is `not excluded (possible handle)` instead, that H0's (h) states `watch`'s 64-element prefix limit, and that the performance mode each hold, each cadence `watch` (steps 5 and 6) and each by-eye look for V ran in is read back through `gjson <R8 var>` when R8 is known rather than trusted from memory, and must show that run's or look's mode; a watch run or V look without that reading cannot serve H0, and a concluded H0 or H2 quotes its limit (the option's value, not its applied effect; one control name per enumeration).
...
- `docs/`: Reverse-engineering research logs, memory audits, and drop rate analysis.
  ...
  - `minimap-smoothing-research.md`: ForgePact issue #19 (minimap markers jump about once a second in the game's performance mode). Records the static search (no SDK name contains "performance"; the complete minimap script family and `objMinimap` closures), the hypotheses (H0-H3) with the evidence each requires and human decisions D1-D6, the game-independent interpolation core, the `mmprobe` instrument, the one-round live procedure with a control per step (every hook-free measurement runs before the first hook), **Deciding the hypothesis** (the required-evidence table: H2 and H0, which close the issue, need positive evidence - never a script row's gap or `mmprobe show` alone - and a silent instrument concludes "not observed" with the recorded event-entry-point detour follow-up), and the **Results** table (R1-R10, the `V` row - whether the jump is seen by eye with performance mode on and gone with it off, recorded but never sufficient - C, and the `H` row naming the hypothesis and its evidence letters) the live session fills in. H2 and H0 additionally require the (c') control: the same record, field and route seen to displace the player's marker through some write point, because an undisplaced hold on a field the draw does not read looks identical under every hypothesis; an H2 whose one routine both refills and draws admits no such write point and stays not observed. For H2 only the mode-on `hold … at <R1 row> post` (step 12) satisfies (c') - a displacement by the frame-end `hold` with performance mode off ties the record only to the mode-off draw, is recorded in R10, and never satisfies H0's (g) either unless the mode is on. Round 1 (the split-fix review, `test_research_doc_h2_exclusion_fails_closed`) rebuilds (c'') as a fail-closed **draw-side store ledger** (R11): a fixed enumeration (every `objMinimap` variable; every `Player_obj` variable and global whose name contains minimap, marker, radar, icon or blip, from `gnames`/`gjson`) whose entries are `excluded (held)`, `excluded (scalar)` or `not excluded (<reason>)` - anything this build cannot measure or write stays `not excluded`, never excluded by default, superseding the round-0 rule that excluded a candidate only by its watched cadence (silently passing a ds_grid/ds_map/buffer id, which `watch` without `list` can never see change, and a struct container, which `hold` cannot write). A concluded H2 quotes the ledger's limit. Because `objMinimap` carries `minimapDiscoveredGrid` (a ds_grid; see `map-reveal-research.md`), H2 is expected to be `not observed (draw-side store not excluded: ...)` in the one live session, leaving issue #19 open until a store-agnostic instrument exists - a deliberate trade-off, not a defect. H0 (`test_research_doc_h0_fails_closed`) is tightened the same way: (g) counts only a displacement with performance mode **on**; (h) requires both `watch` runs to have actually measured a cadence (at least two changes, `read failures=0`); (i) requires V to have been recorded at step 1, else H = `not observed (V not recorded)` - and still contradicts H0 outright when V shows the dots jumping on and moving off (H = `not observed (V contradicts H0)`). Also records that session 7 of the Pet Quest Collector research found raw object-event names not resolving on three other objects, so `objMinimap`'s own event rows are expected (and measured at step 9) to print not found and only named-script rows can count. Round 2 (the round-1 review) closes two more uncontrolled inputs found by auditing every input a closing verdict consumes: the ledger's enumeration (`mmprobe vars`, `gnames`) is now controlled through a second builtin on names already known to exist (`oget`/`iget`/`gjson`/`cb instance_number`), and any control failure fails the whole ledger closed as `not excluded (enumeration uncontrolled: <command>)` rather than reading an empty or partial print as complete; and the `excluded (scalar)` status's non-negative-integer test is hoisted over both its branches, so a handle-shaped value is `not excluded (possible handle)` whether or not it changed. Consequence stated in the doc: this makes H2 unreachable through the ledger in every session on this build, not only this one - `minimapShowMonsters` and every other 0/1 option flag the enumeration carries reads as a possible handle - so #19 stays open until an instrument observes the draw directly, which is the recorded (not built) follow-up. `phase0-status: complete` after the 2026-09-16 session, which recorded H = not observed: performance mode (`minimapTurboPerformance`) stretches the `DrawMinimapDynamic` burst interval from 16 to 109 frames, markers are drawn per object type onto a surface, and no per-marker position list exists; nothing player-visible has shipped.
...
| `py -m unittest discover -s tests -v` | `ForgePact/` | PowerShell / CMD | Python 3.10+ | Executes all 545 Python contract tests (including the native behavior harnesses, which skip without a C++ toolchain). | Read-only test execution; all tests pass | Verified 2026-09-18 |
...
- Example commands:
  ...
  - `mmprobe hook [substr ...]` / `show` / `reset` / `verbose on|off` / `vars [Obj]` / `snap global|<Obj>` / `diff` / `watch <Obj|global> <var> [frames] [list]` / `hold <Obj|global> <var> <index> <field> <delta> [frames] [list] [at <label> [pre|post]]` / `hold stop` / `dslist <id> [n]` (**research build only**, not in `kPlayerCommands`): the minimap smoothing Phase 0 instrument. `hook` native-detours (`MmCreateHook`, after `AddrIsExecutableInModule`) every minimap/options candidate from `docs/minimap-smoothing-research.md` plus `CheckPlayerInteraction` as the positive control; substrings restrict the set to bisect a crash. The 27 `objMinimap_<Event>` rows are **expected to print `not found`** (session 7 in `ForgePact/docs/pet-quest-collector-research.md` found 22 raw object-event names on three other objects all not found; `objMinimap`'s own rows are measured by this hook), so only the named-script rows can count; event rows log `self` only, and a name that resolves to raw code is refused rather than read as a script. **Reading `show`:** each row is `calls`, `since` (since the last show), `lastGap`, and the two commonest gaps between consecutive calls in frames - `gap=60 x 9` is "once every 60 frames", `gap=1` is every frame, `gap=0` is several calls in one frame; compare against the header's frames-since-reset and `game_get_speed`. The last line is the control: `CheckPlayerInteraction: calls=0` voids every row above. `watch` measures the same cadence from `FrameCallback` without any hook, as the cross-check; it skips (and counts) frames whose read failed instead of recording them as changes, and `list` compares a ds_list's size and contents rather than its id. `snap`/`diff` names the performance-mode variable by diffing scalars around a toggle (control: `minimapShowMonsters` toggled in the game's menu; an empty result reads "not observed among scalar globals"). **`hold` is the one write:** it adds `delta` to one numeric field of one marker record (element `index` of the container) for a bounded number of frames, reads the first write back (`route=… typeof=… at=… readback ok|MISMATCH`), counts the writes the game overwrote (a third cadence measure, and proof the container is live), and restores the original value at the end - **and on an abort after a write** (`stopped - … (wrote=W restored=…)`). The element's kind decides everything before any instance check: an array takes the array route; a real, int32, int64 or reference goes through `instance_exists` (the kinds `HhResolveInstance` passes it) and is refused either way; an object is asked `typeof` - `"struct"` takes the struct route, `"method"` is refused, anything else goes through `instance_exists` and is refused; every other kind (string, `undefined`, pointer, bool) is refused with no runtime call at all, because a GML type error inside that builtin is a fatal dialog, not a catchable exception. Whether this runner reports an instance-backed object as `"struct"` is unmeasured, which is why the first line prints `typeof=`. Without `at`, the write happens in the same research frame tick as `watch` (one of the two at a time), i.e. at the **end** of the frame - so a refill that runs before the next draw clobbers it, and an undisplaced frame-end hold proves nothing. `at <label> [pre|post]` (default `post`) makes the same write inside an already-detoured named-script row, before or after the game's own call (`hook it first` otherwise; event rows are never write points); `at <refill row> post` lands after the game's refill and before the next draw, which makes it the positive control that the draw depends on this record, field and route - for H2 only `at <refill row> post` with performance mode on counts; a mode-off displacement only ties the record to the mode-off draw. An `at` hold pins the target object's instance id at Start (`InstanceIdOf`, the kinds `HhResolveInstance` uses) and reads/restores through that pinned instance rather than a re-resolved one; if a zone change replaces it, the hold stops cleanly with `minimap instance changed (... id was ..., now ...)` - restore attempted - instead of writing into a different instance's record. The reentrancy flag (`g_MmProbeHoldInApply`) is cleared by a local `InApplyScope` scope guard, not a standalone statement, so an exception inside `MmProbeHoldAbort` cannot leave it stuck. A hold never displaced proves nothing about the draw unless the same record, field and route was displaced through some write point. `vars` lists custom instance variables only (`variable_instance_get_names`; a built-in such as `x` is never listed) and says so on every call, naming itself uncontrolled when the count is zero - an empty or partial enumeration print must not read as a complete one. `hold … at`'s pin block separates `unknown object` (the resolved index is negative) from `no … instance to pin` (the object exists but has no live instance) from `<obj> instance is <kind>, not an instance kind` (an instance was found but is not one of the kinds `HhResolveInstance` accepts), instead of collapsing all three into one message. Every other subcommand is read-only.
...
- Minimap Smoothing Research (issue #19, Phase 0 pending): `../../../ForgePact/docs/minimap-smoothing-research.md`
