...
14. **The kill drops moved ahead of the original kill proc (1.4.3) - in-game confirmation pending:**
    ...
    - **Why:** players reported crashes in fights with the Angelic / Unholy slider at x100 (`angelicdrop 76`) on 1.4.1. The only step that runs per hit and nowhere else is the spawn with the enemy as `self`, and it ran after the game had cleaned that enemy up. Spawning while the enemy is live is the only `self` with a live positive control; the global instance and the local player as `self` were rejected as unmeasured, and deferring to `FrameCallback` as a context no spawn has been measured in.
