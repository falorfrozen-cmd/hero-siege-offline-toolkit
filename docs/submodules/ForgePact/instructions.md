...
- `tests/`: Automated Python contract test suites validating plugin source invariants and panel logic without requiring a running game instance.
  ...
  - `test_incarnation_gems_contract.py`: Pins the gem mod's wiring: the one native `DropGems` hook by SDK constant, installed once a player exists; the seed swap before the outermost `CreateItemNew` and only inside the drop scope; the dress on the finished item before Item Truth records it; the table candidates skipping every other post-process; the frame budgets; the three player commands with the research verbs kept out; the mod state; the panel's two switches (default on - the named exception in `test_release_hook_contract.py`) and the filter (its list equal to the research doc's affix table, `/api/set` validation, the launch and live commands, the grouped Mods card).
...
- **Default on, by the owner's call** (2026-09-25): the one named exception in
