...
| `py -m unittest discover -s tests -v` | `ForgePact/` | PowerShell / CMD | Python 3.10+ | Executes all 813 Python contract tests (including the native behavior harnesses, which skip without a C++ toolchain). | Read-only test execution; all tests pass | Verified 2026-09-20 |
...
16. **The toggle-skill active indicator's ownership and timing are inferred, not proven, on two axes (issue #11, Track B, `toggleborder`):** ForgePact ships offline-only (repo-root `AGENTS.md`, "this is the rule of ForgePact"), so neither axis below is a shipping risk — only a limit on what has actually been observed.
