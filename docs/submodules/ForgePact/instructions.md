...
- `autoprospect bag 1` / `autoprospect bag 0`: Auto-prospect's sub-option (issue #9, Stage C; panel key `mod_auto_prospect_bag`, nested under `mod_auto_prospect` the way `map_reveal_packs` sits under `map_reveal`; **on by default** in the core and the panel, so `build_cmds` emits `autoprospect bag 0` after `autoprospect 1` only when it is off, and turning the parent on live restates it; `autoprospect 1|0` leaves it alone).
  ...
  - `craftprobe hook [substr ...]` / `arm [budget=N] [inroute] [substr ...]` / `show [all]` / `reset` / `bag|stash|recipe [substr ...]` / `var <Obj|global> <nth> <name|a.b.c|*> [json]` / `var id:<n> <name|a.b.c|*> [json]` / `var <Obj> <nth>|id:<n> * from=<i>` / `find <Obj> <nth>|id:<n> [from=<i>] <text>` / `node id:<n>|<Obj> <nth>|stash|bag|socket [id:<n>]` / `node var <Obj|global> <nth> <a.b.c>` / `node var id:<n> <a.b.c>` (each `node` then takes any of `a1=<v>` `self=id:<n>` `class=<c>`) / `store [substr ...]` / `store names [substr ...]` / `backing on [substr ...]|off|dump|clear` / `dump` / `call <Row> <Obj> <nth>|id:<n> [args ...] confirm` / `callm <Obj> <nth>|id:<n> <struct> <member> [args ...] [bind] confirm` / `set <struct> <member> <number> confirm` (struct also `kept:<row>[.a.b]`) / `inject <class> <b> <extra>|off` (**research build only**, not in `kPlayerCommands`, dispatched from `HandleCraftCommand`): the issue #14 instrument, Phase 1k build; see `ForgePact/docs/crafting-materials-research.md` § Instrument, its "Phase 1b additions", "Phase 1c readers", "Phase 1e instrument", "Phase 1g instrument", "Phase 1h instrument", "Phase 1i instrument", "Phase 1j instrument" and "Phase 1k instrument".
    - A bare `craftprobe` prints the usage, whose first line is the build's marker, `craftprobe: phase1k rows=281 - ...`; a live session checks it before anything else counts (the Phase 1j build printed `phase1j` with 278 rows; the Phase 1i build printed `phase1i` with 254 rows; the Phase 1h build printed `phase1h` with the same 254 rows; the Phase 1g build printed `phase1g` with 252 rows; the Phase 1e build, which Phase 1f also ran, printed `phase1e` with the same 252 rows; the Phase 1c build, which Phase 1d also ran, printed `phase1c` with 223 rows; the Phase 1b build `phase1b` with 202; the Phase 1 build, `aa0c72a`, has 97 rows and no marker).
    ...
    - Phase 1k (the by-name partial take; `### Phase 1k instrument`) adds three rows, 281 in all, each through its `hs-game-sdk` constant - `InitItemFromJson`, `ReCreateItem` and `ParseItemToGrid`, the game's loader route that makes an item without its constructor - so `hook` expects `279 detoured, 0 failed, 2 held by mapkeep`; the marker becomes `phase1k`; `callm ... [args ...] bind confirm` re-binds the member to the struct it was read from through the runtime's own `method` builtin, by name, after every existing precondition and before the one `script_execute`, which gets the bound value (on the local Ghidra reading, Live 1j's four calls threw because the item's methods are stored unbound, so the instance ran as their self; `HashRouteMethod`'s shipped `+method` route is the precedent, never measured to change a hash), the reply adding `bind=yes` and `method_get_self` before and after (read only, unverified on this runtime); and `set kept:<row>[.a.b] <member> <number> confirm` writes into a row's kept return (`backing on <row>` first; a by-name `call`'s return is kept too), refused naming what was supplied when that row keeps nothing. Without `bind`, `callm` is Live 1j's.
