...
16. **Nothing here can select a character yet, and the instrument that would measure whether anything can has not been run (2026-09-20):**
    ...
    - **The controls that decide whether a future negative means anything.** Two of them, both written down before the session so neither can be skipped and rationalised afterwards. (1) A human holds a key while the agent reads the engine's and the operating system's key state; if that read is not true, the key instrument is blind and the two input candidates are *unmeasured*, not negative. (2) `menuprobe list` must find button instances at the main menu; menu-room instances have never been shown to be enumerable on this runner, so an empty list means the event and script candidates were never measured either. `AGENTS.md`, "Prove the Instrument Before Trusting a Negative Result".
