...
- **Source Availability:** Full application source is present (Python control panel `src/forgepact.py`, C++20 native mod plugin `plugin/ModuleMain.cpp`, build scripts `plugin_build/build.bat` and `build_release.py`, Python contract tests `tests/`, and reverse-engineering research notes `docs/`). **The one exception is the distributed `YYToolkit.dll`:** `yytoolkit-modified/` holds a notice and two whole-file copies that do not account for everything in that binary (see the pin table under "The build half"), and the source of truth for the toolkit's modified YYToolkit is the hub's patch series, [`third_party/yytoolkit/`](../../../third_party/yytoolkit/README.md), not this repository.
...
- `modfiles_shipped/`: Shipped binaries deployed to the game's `bin/` directory upon mod installation.
  ...
  - `YYToolkit.dll`: GameMaker runtime interface library, a modified build of YYToolkit v4.0.1 (AGPL-3.0). The file pinned today is SHA-256 `bb113eef…` (904,192 bytes). Its notice lists two changes — a runner-interface disk cache and the `ExecuteIt` hook left uninstalled — but the binary contains more than that, and the source it was built from was not kept; see the provenance row in the pin table below. Its documented replacement is the hub's patch series in `third_party/yytoolkit/`, which this repository does **not** pin yet.
...
- `yytoolkit-modified/`: The AGPL-3.0 notice and two whole-file copies (not patches) that accompany the `bb113eef…` DLL. **Not the complete source for that binary, and not where a change to YYToolkit is made** — that is the hub's [`third_party/yytoolkit/`](../../../third_party/yytoolkit/README.md). Until a ForgePact PR moves the DLL pin, these files remain the statement for what players receive, and they are incomplete.
  - `NOTICE.md`: Rationale, file listing and a rebuild recipe for the two files below. It says everything else is unmodified upstream, which the binary's own strings contradict; its recipe (`MSVC toolset 14.50`, `cl /std:c++latest /MD /LD`) does not match the DLL's PE header (linker 14.51), and it omits `/O2` although the DLL was **likely** built with it: a plain `cl /O2` build of the documented tree is 894,976 bytes against the DLL's 904,192, one without `/O2` is 1,180,160, and the DLL has pooled strings and no `.rsrc` section, unlike a build through upstream's `vcxproj`.
  - `Generic-RunnerInterfaceNew.cpp`: Adds a disk cache (`<exe>.yytkcache`) and a density-sorted `.text` page pre-filter so the RunnerInterface search does not disassemble all of `.text` on every launch. The pre-filter is in the file but was never listed in `NOTICE.md`. On a cache hit this version plants the mid-function hook at the address read from the file, validating nothing but the executable's size — and `src/forgepact.py`'s `ensure_ri_cache` pre-seeds that file from a table of hand-measured RVAs keyed by exe size. Hub patch `0001` keeps the file as a hint for which pages to scan first and always takes the hook address from the scan; the legacy three-number line stays readable, so the panel's pre-seeding keeps working and is then harmless rather than load-bearing (follow-up for this repository: the table can be deleted once the pin moves).
  - `source/YYTK/Hooks.cpp`: Leaves the `ExecuteIt` hook (`EVENT_OBJECT_CALL`) uninstalled; `EVENT_FRAME` via `HkPresent` is unaffected. The reason `NOTICE.md` gives — the hook corrupting instance references and producing `Unable to find any instance for object index` — is contradicted by `docs/S10-special-content-notes.md`, which records the same error with the hook removed. What stands: no plugin in this toolkit consumes `EVENT_OBJECT_CALL`, and project notes record the per-event hook crash-looping on Season 10 (not re-measured since). Hub patch `0003` makes the off state a named build switch, logs it once, and refuses the registration with `AURIE_UNAVAILABLE` instead of accepting a callback that can never fire.
...
- **YYToolkit Headers and third-party binaries:** the `YYToolkit/`, `Aurie/`, `FunctionWrapper/`
  ...
  (forgepact-release.yml)" below for the full pin table. The `YYToolkit.dll` it places is
  still the `bb113eef…` file; a DLL built from the hub's `third_party/yytoolkit/` series with
  `tools/build_yytoolkit.py` is **not** one of the eleven pins, and moving the pin to one is
  a ForgePact PR of its own (see the pin table's provenance note).
...
| `py -m unittest discover -s tests -v` | `ForgePact/` | PowerShell / CMD | Python 3.10+ | Executes all 586 Python contract tests (including the native behavior harnesses, which skip without a C++ toolchain). | Read-only test execution; all tests pass | Verified 2026-09-18 |
...
**What the pinned `YYToolkit.dll` is, and what its notice leaves out (recorded
2026-09-19).** The file is SHA-256
`bb113eefc9a5d485231ced1dc85d773dbc6b762ee680214851c56541359ad297`, 904,192
bytes; HS-Offline-Tracker's `aurie-loader/YYToolkit.dll` is the same file
...
all read from the header and the strings, nothing disassembled.
`yytoolkit-modified/NOTICE.md` documents two changes and says everything else
is unmodified upstream. Strings in the binary show that is not so:
...
- the lea/mov page pre-filter, which *is* in the committed
  `Generic-RunnerInterfaceNew.cpp` but was never listed in `NOTICE.md`.
...
complete corresponding source for the binary players receive today**, and
"rebuild it from `yytoolkit-modified/`" is not a recovery path: a fresh build
of that documented tree did not get the game started. Its `YYToolkit.log` ends
...
on one machine and its log markers checked in the DLL (with a negative
control: `verify-dll --dll` run against `bb113eef…` fails, naming the markers
that file lacks). It has **never been launched against the game** — whether it
gets past the startup fault, and whether it changes the in-play lag, is not
measured. Moving this row to a build of that series is a ForgePact PR that
does not exist yet, and it has to land together with HS-Offline-Tracker's:
both tools install to `mods/aurie/YYToolkit.dll` with overwrite semantics
(`src/forgepact.py` copies unconditionally), so whichever installs last wins,
and shipping the new DLL in only one of them lets the other put the old one
back. Until then this row, and the `yytoolkit-modified/` notice it points at,
stay as they are — incomplete, and said to be.
...
6. **Diagnose stalls with the built-in watchdog, do not guess:**
   ...
   - **"Noise" describes who causes those entries, not what they cost (added 2026-09-19).** Each one is a full YYToolkit error report: a stack trace, a symbol for every frame, the module list and a log write — and upstream's symbol resolver rebuilds its map of every script and builtin once per stack *frame* of every report. That is read from upstream's source and is structurally certain; **its cost on this game is not measured**. A separate session reported about 142 such reports in about two minutes of play with YYToolkit alone lagging the game (vanilla, and offline without mods, smooth) — reported, not re-measured here. Hub patch `0005` in `third_party/yytoolkit/` reports each distinct message once, counts repeats, and writes `[hs] YYError report cost` / `[hs] YYError summary: total=` lines so the first launch of that DLL measures it. Until that launch happens, whether the reports are what lags the game is **not observed**, and the other candidates are neither measured nor ruled out: the console YYToolkit allocates (synchronous writes if the runner prints each error to it), per-call allocations in plugin-side `CallBuiltinEx`, and — for any build made through upstream's project file, which compiles `Release|x64` with optimisation disabled — the optimisation level, which patch `0006` sets explicitly.
...
- **Moving the `YYToolkit.dll` pin (follow-up, not done):** a ForgePact PR that (1) repoints `tools/toolchain-pins.json` at a launch-gated build of the hub series, with its SHA-256 and the hub commit as provenance — the pin count stays eleven; (2) rewrites `yytoolkit-modified/NOTICE.md` to point at the hub directory and deletes the two whole-file copies in the same PR; (3) corrects `CREDITS.md` and `README.md`, which still say "one modified source file"; (4) ships the notice with the binary — `build_release.py` copies only `README.md`, `CREDITS.md` and `LICENSE` into the bundle today; (5) carries `release-notes-vX.Y.Z.md`, claiming a startup or lag fix only if it was measured; (6) deletes `ensure_ri_cache`'s hand-measured RVA table from `src/forgepact.py`, which patch `0001` makes unnecessary. It lands together with HS-Offline-Tracker's equivalent, for the overwrite reason given under the pin table. Afterwards, update the pin-table row and the repository-map entries in this guide.
...
- Notice for the currently pinned `bb113eef…` DLL (incomplete — see the pin table): `../../../ForgePact/yytoolkit-modified/NOTICE.md`
