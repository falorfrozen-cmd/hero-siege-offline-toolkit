...
| `py -m unittest discover -s tests -v` | `ForgePact/` | PowerShell / CMD | Python 3.10+ | Executes all 773 Python contract tests (including the native behavior harnesses, which skip without a C++ toolchain). | Read-only test execution; all tests pass | Verified 2026-09-20 |
...
- Example commands:
  ...
  - `toggleborder 1` / `toggleborder 0` / `toggleborder stat`: shipped toggle-skill active indicator (issue #11, Track B). While on, `ToggleIndicatorDraw()` runs right after `HhDrawHeadLabels()` inside the existing `DrawHudBuffs` hook (no new hook installed): it re-reads `ToggleIndicatorRead()` every draw (ownership from each `White_Mage_Soul_Spurn_AOE_obj` instance's own `isMyClient`, no local-player lookup) and requires the Purgatory marker (`purgatory` numeric > 0) - session 4 measured a plain, non-Purgatory cast of Soul Spurn otherwise flashes the outline for ~146 draws (`docs/toggle-skills-research.md`, "After session 4"). On a match it finds Soul Spurn's slot (`UI_Hud_Talent_obj` instance 0's own `row0` array, the element whose own `talentId` reads 240) and draws a gold, 3 px outline around its own `navBboxX`/`navBboxY`/`navBboxWidth`/`navBboxHeight`, saving and restoring `draw_get_colour`/`draw_get_alpha`. `toggleborder 0` prints `drawn=`/`on=`/`off=`/`unreadable=`/`noHud=`/`noRow0=`/`noTalent=`/`foreign=`/`drawExc=` - `noHud`/`noRow0`/`noTalent` are the three meaningfully different reasons `ToggleIndicatorFindSlot` can fail to find the slot (follow-up from the indicator's reviews; replaces a single `noSlot=`), and `drawExc` counts a swallowed exception from the outline draw itself. `toggleborder stat` (read-only, stores nothing) prints the same counters plus `enabled=on|off`. Off by default; panel key `mod_toggle_indicator`. See Known Limitations for the co-op inference and the OFF lag.
  - `toggleguard 1` / `toggleguard 0` / `toggleguard stat`: shipped toggle-skill re-cast guard (issue #11, Track A, T1). `1` only arms it; `FrameCallback` installs `HookOneScript("TalentUseClass", …)` (`HookTalentUseClass`, both routes) once the setup gate has passed and `HhResolveLocalPlayer` succeeds, checked once a second - the `relicfilter` shape (Known Limitations item 8), so it is safe to send at launch. While on, a `TalentUseClass` call whose caller's own `object_index` (read with `variable_instance_get` and `N1ObjectIndex`, which accepts the `VALUE_REF` this runner returns) equals `asset_get_index("Universal_Double_Cast_obj")` and whose talent is Soul Spurn's (`kToggleIndicatorTalentId`, 240) is refused: the hook returns without calling the original, the co-op puppet's early-return shape. Everything else passes, and the toggle's state is never read (`docs/toggle-skills-research.md`, "Track A design (D-N1)"). A caller or object that cannot be read passes (fails open) and is counted. `toggleguard 0` clears the flag only; the hook stays and passes every call through. `toggleguard 0` and `toggleguard stat` (read-only) print `refused=`/`passed=`/`procSeen=`/`selfUnreadable=`/`objUnresolved=`/`hook=not installed|installed|TABLE-ONLY`, `stat` also `enabled=`; the research build adds `lastProcRet=` (what the last passed proc call returned). Off by default; panel key `mod_toggle_guard`. See Known Limitations item 17.
...
16. **The toggle-skill active indicator's ownership and timing are inferred, not proven, on two axes (issue #11, Track B, `toggleborder`):** ForgePact ships offline-only (repo-root `AGENTS.md`, "this is the rule of ForgePact"), so neither axis below is a shipping risk — only a limit on what has actually been observed.
    ...
    - **Soul Spurn/Purgatory only.** The talent id (240) is a research-measured literal (no `hs-game-sdk` constant exists for it), and the read targets only `White_Mage_Soul_Spurn_AOE_obj`. No other toggle skill or class is covered.
...
17. **The toggle-skill re-cast guard refuses by caller, not by state, and covers one source of accidental re-casts (issue #11, Track A, `toggleguard`):** ForgePact ships offline-only (repo-root `AGENTS.md`, "this is the rule of ForgePact"); nothing below is framed around co-op.
    - **Every double-cast proc re-cast of Soul Spurn is refused, with or without Purgatory.** The guard cannot tell a Purgatory toggle cast from a plain cast: the toggle's state cannot tell "just turned off" from "never on" (the proc lands 36–56 frames after the press, and the AOE is gone 19 frames after an OFF press), so it keys on the caller alone (decision D-N1; accepted by the user, D-U4). With the guard on, a plain Soul Spurn cast without Purgatory loses its double-cast re-cast. Gating on Purgatory needs its `global.subTalentMap[1].t240` sub-index, which is not identified yet (session 5's optional V8); that is a later follow-up.
...
18. **Hero Siege draws on a whole-pixel grid — any overlay ForgePact draws must use integer coordinates and integer sizes.** Measured live during the sprite-look-probe tuning (issue #11, R round 7): fractional screen positions like `12.5` or `75.1` do not occur; the tester, quoted, `'everything seems to be drawn in a whole number position'`. The fractional values a runtime read hands back for a talent slot (e.g. `navBboxX=385.700006 navBboxY=1711.000000 navBboxWidth=124.700000 navBboxHeight=139.200000`) are an artefact of the number type those fields are stored as, not the art's actual on-screen placement. **Derive a position or size from a read like that, then round to the nearest whole pixel before the draw call** — never pass the raw fractional read straight to `draw_rectangle`/`draw_sprite_ext`/`draw_ellipse_colour`/`draw_rectangle_colour` or any future draw call. `ForgePact/docs/toggle-skills-research.md` ("Sprite look probe") has the tuned example this rule came from (Soul Spurn's slot at this session's HUD scale: a `125 x 125` square at `385, 1712`, evidence for that slot at that scale, not a shipped constant — derived by keying a square to the slot's own `navBbox` height, aligned to its left/top corner, both rounded).
