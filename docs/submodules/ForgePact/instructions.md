...
| `py -m unittest discover -s tests -v` | `ForgePact/` | PowerShell / CMD | Python 3.10+ | Executes all 611 Python contract tests (including the native behavior harnesses, which skip without a C++ toolchain). | Read-only test execution; all tests pass | Verified 2026-09-18 |
...
16. **Auto-prospect runs the game's own Prospect at a moment the game did not choose - an accepted risk (2026-09-18):**
    ...
    - **What is unproven.** Whether a click-in's cell fill can land in an earlier frame than its hook is not measured, and a drag-in's timing is not observed; the core is built not to depend on either (an insert counts when the filled count is above the last *settled* count, not the count one frame earlier), and Phase 3 S1 brackets a drag-in. A drag that empties its source cell while held and fills it again on drop would read as an insert; with the mod on, only materials and refused items sit in the grid, and a materials-only press was measured as a no-op (`P-materials-only`). The `S-*` rows (Phase 3, research DLL then player DLL) are pending.
    - **Diagnostics.** `autoprospect stat` is research-build only (the user's rule that player builds carry no debug tooling). The player build logs `autoprospect: hook installed -> ON` or the table-only refusal, each refusal reason once per session, a failed dispatch once, and `autoprospect: first prospect - invoked=… prospected=…` once - a line naming work done, not armed state.
