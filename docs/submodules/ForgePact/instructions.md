...
- `autoprospect bag 1` / `autoprospect bag 0`: Auto-prospect's sub-option (issue #9, Stage C; panel key `mod_auto_prospect_bag`, nested under `mod_auto_prospect` the way `map_reveal_packs` sits under `map_reveal`; **on by default** in the core and the panel, so `build_cmds` emits `autoprospect bag 0` after `autoprospect 1` only when it is off, and turning the parent on live restates it; `autoprospect 1|0` leaves it alone).
  ...
  - `craftprobe hook [substr ...]` / `arm [budget=N] [substr ...]` / `show [all]` / `reset` / `bag|stash|recipe [substr ...]` / `var <Obj|global> <nth> <name|a.b.c|*> [json]` / `var id:<n> <name|a.b.c|*> [json]` / `node id:<n>|<Obj> <nth>|stash|bag` / `backing on [substr ...]|off|dump|clear` / `dump` / `call <Row> <Obj> <nth> [args ...] confirm` (**research build only**, not in `kPlayerCommands`, dispatched from `HandleCraftCommand`): the issue #14 instrument, Phase 1b build; see `ForgePact/docs/crafting-materials-research.md` § Instrument and its "Phase 1b additions".
    - A bare `craftprobe` prints the usage, whose first line is the build's marker, `craftprobe: phase1b rows=202 - ...`; a live session checks it before anything else counts (the Phase 1 build, `aa0c72a`, has 97 rows and no marker).
    - `hook` native-detours all 202 candidate rows (resolved by name, refused outside `Hero_Siege.exe` before `MmCreateHook`) - Phase 1b added the grid-input/drag, add/stack, socket-family, stash-tab-buy and node/tab rows plus every Create closure of nine grid, inventory and stash objects. `CheckPlayerInteraction` is the positive control; `arm`/`show` budget and report the per-call self/other/arguments/return lines.
    - The readers are hook-free and write nothing. `var` follows a live `ref instance N` by name: `object_get_name` of its `object_index`, its id and its variables, each further instance reference one level deeper (depth 2, 12 instances, a visited set), and only after `instance_exists` answers true; a `ref ds_grid|ds_list|ds_map` is read only after `ds_exists` with its own type answers true; `id:<n>` starts from an instance number, `a.b.c` walks a dotted path, `*` lists every variable; a method value is described, never called; no struct-layout read. `node` reads a container's `nodeGrid` (size, filled/empty, distinct `nodeFingerprint`s), makes one by-name `GetItemFromFingerprint(fp, 0)` per fingerprint (capped at 64 per command, through the same helper as `call`'s `fp:`), prints `itemType`, `itemDefinitionStruct.b`, the class suffix and every numeric member that may be the stack count - named exactly `o` (the save's stack key, which the item struct shares with `b`) or like `stack`/`amount`/`count`/`qty`, and, when the definition struct has none, all its numeric members (24 at most) - then a `sum class=<c> b=<b>` line per pair; a lookup miss prints what was supplied (`self=<object> id=<n>, a1=0`); `stash` reads each live instance reference of the open stash window, `bag` each `UI_Inventory_Grid_obj` (the known-good control, which passes only when a sum equals a bag stack the owner states by eye), and an instance without `nodeGrid` prints its variable names.
    - `backing` keeps what the game's own getter calls returned (never invokes one); for `GetInventoryArray` and `CountInventoryItem` it also keeps the latest return per distinct first argument (8 signatures), and `backing dump` writes `cp_backing_<row>_arg<k>.json` per signature. `call` is the one write - a single by-name `script_execute` of one plain-script row behind the literal `confirm`, refused before the call on any missing precondition.
