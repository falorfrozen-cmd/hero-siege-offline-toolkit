...
- **Source Availability:** Full application source is present (Python control panel `src/forgepact.py`, C++20 native mod plugin `plugin/ModuleMain.cpp`, build scripts `plugin_build/build.bat` and `build_release.py`, Python contract tests `tests/`, and reverse-engineering research notes `docs/`). **The one exception is the distributed `YYToolkit.dll`:** `yytoolkit-modified/` holds a notice and two whole-file copies that do not account for everything in that binary (see the pin table under "The build half"), and the source of truth for the toolkit's modified YYToolkit is the hub's patch series, [`third_party/yytoolkit/`](../../../third_party/yytoolkit/README.md), not this repository. A branch that moves the pin to a build of that series and replaces `yytoolkit-modified/` with a pointer at it (`claude/yytoolkit-hs1-distribution`) is open as a pull request on this repository, but it is **not merged to `origin`, and no ForgePact release has shipped it** (the hub library release its pin names is itself now published — see the pin table's provenance note for the distinction).
...
- `yytoolkit-modified/`: On `origin`, the AGPL-3.0 notice and two whole-file copies (not patches) that accompany the `bb113eef…` DLL. **Not the complete source for that binary, and not where a change to YYToolkit is made** — that is the hub's [`third_party/yytoolkit/`](../../../third_party/yytoolkit/README.md). A local, unmerged branch has already rewritten this directory to a pointer at the hub series (`NOTICE.md` plus `YYToolkit-BUILD-INFO.json`, no whole-file copies); until that branch is merged and released, `origin`'s two whole-file copies remain the statement for what players receive, and they are incomplete.
...
- **YYToolkit Headers and third-party binaries:** the `YYToolkit/`, `Aurie/`, `FunctionWrapper/`
  ...
  (forgepact-release.yml)" below for the full pin table. On `origin`, the `YYToolkit.dll` it
  places is still the `bb113eef…` file; a DLL built from the hub's `third_party/yytoolkit/`
  series with `tools/build_yytoolkit.py` is pinned on a local, unmerged branch
  (`claude/yytoolkit-hs1-distribution`), not on `origin` yet (see the pin table's provenance
  note).
...
one machine. Moving this row to a build of that series is open as a pull
request (`claude/yytoolkit-hs1-distribution`) — **not merged to `origin`,
and no ForgePact release has shipped it** — which points
...
the old one back. Until then, `origin`'s pin table row above, and the
`yytoolkit-modified/` notice it points at, stay as they are — incomplete, and
