...
- `tests/`: Automated Python contract test suites validating plugin source invariants and panel logic without requiring a running game instance.
  ...
  - `test_angelic_probe_contract.py`: Pins `angelicprobe`, the research instrument for issue #64 (where the game's own Angelic roll picks a unique; the research doc is listed under `docs/` below).
    ...
    - `PlayerBuildUnchangedTests` (skipped unless `FORGEPACT_TEST_BASE_DIR` names a directory holding `137a403`'s `ModuleMain.cpp` and `DropManager.hpp`) asserts the player build of `DropManager.hpp` and of `HookAngelicChance`, `FindAngelicGate`, `OpenAngelicGate`, `CloseAngelicGate`, `InstallHook` and `Hook_EnemyDestroyKillProc` is unchanged, and that `InstallItemInspectHooks` and `DropManager::InstallHooks` are unchanged in full.
