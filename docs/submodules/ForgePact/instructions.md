...
- `plugin/`: Native GameMaker mod plugin implementation.
  ...
  - `include/ForgePact/CraftMatsMod.hpp`: The crafting-materials decision core (issue #14, Phase 0; nothing player-visible yet). Game-independent - standard headers only, no runtime interface - so `tests/craft_mats_harness.cpp` compiles it whole. Per material a craft needs: need N, bag count k, stash-material-tab count s -> take min(N-k, s) from the stash's material tab (its one source, by design) only when the switch is on and k < N; a count the decision needs that cannot be read refuses the whole press and is named once; a take the game declined leaves the mod on; a tab that changed without the game's success answer, or a success the re-read cannot confirm, turns the mod off for the session. No adapter feeds it yet: which game routine counts and consumes is Phase 1's question (`docs/crafting-materials-research.md`). Phase 1e adds `CraftMatsKeptMap`, the kept stash map's currency rule - current only when the game's own `GetItemMap(9)` return refreshed it after the latest character load or room change, never on `ds_exists` alone (map indices are reused) - pinned by the harness's `kept_map` baseline and target scenarios and fed, so far, only by the research build's `mapkeep`.
...
- `autoprospect bag 1` / `autoprospect bag 0`: Auto-prospect's sub-option (issue #9, Stage C; panel key `mod_auto_prospect_bag`, nested under `mod_auto_prospect` the way `map_reveal_packs` sits under `map_reveal`; **on by default** in the core and the panel, so `build_cmds` emits `autoprospect bag 0` after `autoprospect 1` only when it is off, and turning the parent on live restates it; `autoprospect 1|0` leaves it alone).
  ...
  - `craftmats 1` / `craftmats 0` (+ `craftmats stat` in the research build only; dispatched from `HandleCraftCommand`; **not yet in `kPlayerCommands`**, so the player build answers `command unavailable`): the switch of the crafting-materials core (issue #14, `plugin/include/ForgePact/CraftMatsMod.hpp`), off by default. Phase 0 wires nothing to crafting - no hook, no call, nothing on the frame path - and `craftmats 1` says crafting is unchanged. The panel has no switch for it yet.
