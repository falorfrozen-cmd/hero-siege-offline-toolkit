...
| `py -m unittest discover -s tests -v` | `ForgePact/` | PowerShell / CMD | Python 3.10+ | Executes all 487 Python contract tests (including the three native behavior harnesses, which skip without a C++ toolchain). | Read-only test execution; all tests pass (2 skipped) | Verified 2026-09-18 |
...
14. **The kill drops moved ahead of the original kill proc (1.4.3) - in-game confirmation pending:**
    ...
    - **Pending live check:** first a control on the unmodified 1.4.2 plugin that reproduces the crash (without it, "no crash after the fix" shows nothing), then `angelicdrop 1`, kill 10+ monsters, `angelicdrop status` (expected: no crash, `drops=` = `rolls=`, `fails=0`), and `sigdrop 100 100` over 4+ kills then `sigdrop status`, `sigdrop vanilla`. For both, also check that each drop is visible on the ground and can be picked up: `drops=` counts a successful `SpawnAngelicItem`/`SpawnSignatureItem` return, not a surviving item, so if the original kill proc removed an item spawned with the still-live enemy as `self`, the counters would read clean while nothing lies on the ground. If the 1.4.2 control does not crash, the release notes must call this a precaution rather than a fix.
