...
- **Purpose & Scope:** Standalone offline control panel and native runtime hook plugin providing runtime modifiers for Hero Siege single-player sessions. Controls monster density, special content spawns (Rift Portals, Battlefields, Cursed Orbs, Chaos Tower, Shadow Realm, etc.), drop rate multipliers and gated drop families (Keys, Relics, Angelic/Unholy uniques), quality-of-life mods (relic drop pool filter excluding maxed 10/10 relics, orb pickup radius, pet-driven quest item collection), player/combat stat scaling, full map reveal (fog, plus an optional pass that makes each new zone's spawners create their packs on arrival so monsters appear on the revealed map), and custom forge mechanics (Headhunter, Tyrant's Crown, Beacon, Item Editor base stat export) without permanently altering save files or the base game executable. Integrated with `hs-game-sdk`.
...
- `tests/`: Automated Python contract test suites validating plugin source invariants and panel logic without requiring a running game instance.
  ...
  - `test_angelic_probe_contract.py`: Pins `angelicprobe`, the research instrument for issue #64 (where the game's own Angelic roll picks a unique; the research doc is listed under `docs/` below).
    - `angelicprobe` is research-build only (the literal, comments included, disappears from `strip_research_blocks`), absent from `kPlayerCommands`, and occurs once in `ModuleMain.cpp`, inside `HandleAngelicProbeCommand`, which `RunCommand` calls on the line after `HandleMenuProbeCommand` (C1061; the top-level `else if` count stays 121).
    - The row table is exactly the seventeen candidate ids, each naming its script as `SdkShortScriptName(HeroSiege::Scripts::gml_Script_<Name>)`; the five DropManager rows carry `kApRollHeldByDropManager` and their hook's name, the two loot-creation rows carry `g_Orig_LootGroundCreate` / `g_Orig_LootGroundCreateFromItem` and theirs (the `tgprobe` existing-hook shape), and DropManager exposes those five saved originals only through a research-only accessor.
    - The `angelic-chance` row reuses `HookAngelicChance`/`g_OrigAngChance`/`fp_angch` with `HookOneScript`'s `nativeOut` (the `"DropItemAngelicChance", "fp_angch"` literal pair still occurs twice); the probe's code holds no `Rva`, no `HookOneScriptTable`, no module-base-plus-literal arithmetic and no `CallGameScript`, its one `MmCreateHook(` sits inside the braced then-block of `if (AddrIsExecutableInModule(...))` (the shared `call_is_inside_addr_check_guard`), nothing in the file calls `gml_Script_DropItemAngelic`, and `FrameCallback` names no `Ap[A-Z]` symbol.
    - `BP_ANGELIC_PROBE_SCOPE` is invoked once in `FP_DROP_HOOK`, before its first original call, and is defined to `((void)0)` in the player build; `HookAngelicChance` raises and lowers the angelic-chance depth only inside `#ifndef FORGEPACT_RELEASE` blocks, around every original call including the extra-roll loop; the research `InstallHook` calls `FindAngelicGate()` before `InstallDropMultHooks();` and the player build does not.
    - `show` prints `calls=n/a` for a row with no route and names every route; `list` resolves `Loot_Manager_obj` by name and formats no double with `%f`.
    - `PlayerBuildUnchangedTests` (skipped unless `FORGEPACT_TEST_BASE_DIR` names a directory holding `be3a723`'s `ModuleMain.cpp` and `DropManager.hpp` - the ForgePact `main` commit the #64 branch merged in) asserts the player build of `DropManager.hpp` and of `HookAngelicChance`, `FindAngelicGate`, `OpenAngelicGate`, `CloseAngelicGate`, `InstallHook` and `Hook_EnemyDestroyKillProc` is unchanged, and that `InstallItemInspectHooks` and `DropManager::InstallHooks` are unchanged in full.
    - The doc tests pin the seven headings, CRLF, every candidate and set-aside name in the static search, a row per candidate and per live step `L-1.1`..`L-1.13`, one labelled `## Decision` line per candidate, and that `finding:` names only candidates labelled `works`. `pending` is accepted only while the live record `.claude/workorders/forgepact-angelic-roll-headhunter-live-1.md` does not exist in the hub checkout.
  - `test_menu_layout_contract.py`: Pins `menulayout`, the read-only player command that lists every live instance of thirty-eight candidate menu, stash, bag and talent objects with its window position (`docs/menu-layout-research.md`, `docs/stash-bag-layout-research.md`, `docs/skill-actions-research.md`): `"menulayout"` is in `kPlayerCommands` and dispatched from `HandleMenuLayoutCommand`, called from `RunCommand` beside `HandleProspectCommand` (C1061), not a new `else if`; no `MenuLayout*` helper mentions a hook installer, `CallGameScriptEx`, `script_execute`, `CallBuiltinEx`, `event_perform`, an `Rva`, `GetModuleHandle`, `instance_create`/`instance_destroy` or `variable_instance_set`, and `FrameCallback` mentions none of them; the candidate table is exactly the character-select research's thirteen `GameObject` names plus the stash research's sixteen plus the skill research's nine (`TALENT_OBJECTS`), each an `hs-game-sdk` enumerator; the header, row and footer format strings (the hub's parse contract) in order; the window point read from `window_get_width`/`window_get_height` over `display_get_gui_width`/`display_get_gui_height` by name; the 200-row cap; `release-notes-v1.4.5.md` names the command; and the research doc's five headings and its four `## Decision` lines, which phase 0 filled on 2026-09-21. None may read `pending` now.
  - `test_stash_bag_layout_contract.py` + `menu_layout_value_text.cpp`: Pins what the stash and bag work (toolkit issue #147) added to `menulayout` (`docs/stash-bag-layout-research.md`): the row's optional-field literal is exactly the ordered list `label`, `name`, `slot`, `index`, `page`, `selected`, `uiNodeCallstack`, `activationArgs`, `enabled`, `tabNumber`, `tabType`, `stashTabSelected`, `tabSelected` (joined after live 2: the bag sub-tab state, `bagTabState`), `nodeGridWidth`, `nodeGridHeight`, `gridScale`, `gridName` (`activationFunc` deliberately absent, `text` still last); an array value is routed to `MenuLayoutArrayText` and prints `[a,b,...]` (32 elements, then `,...+N`), a nested array `<array>`; only a string goes through `ToString`, a reference, struct or pointer printing by kind (`<ref>`, `<object>`, `<ptr>`). **Behavioral** half: splices the production formatter out of `ModuleMain.cpp` into `menu_layout_value_text.cpp`, a stand-in runtime that represents arrays, nested arrays, `VALUE_REF` handles, structs and pointers, whose `ToString` throws for anything but a string (so no invented rendering can pass), and can make `array_length`/`array_get` throw, compiles it with MSVC (skips without a toolchain) and checks each case with a non-array negative control. Also: no `MenuLayout*` helper mentions `script_execute`, `CallBuiltinEx` or anything in the character-select `FORBIDDEN` list; the 200-row cap; the research doc's six headings and exactly one line per each of its fifteen `## Decision` keys, none `pending` since live 2 (`moveWholeRoute` and `moveOneRoute` begin `moved to hs-drive-stash-move-research`), and every live-1 and live-2 `## Results` row filled and dated, with a negative control that runs a fabricated section holding one pending row through the same row parse; the `release-notes-v1.4.8.md` bullet mentions the stash, the bag and grid cells; the README's menu layout section documents the new fields and value tokens, and it and the notes both name `UI_Inventory_Drag_obj` as the inventory's drag object without claiming it holds the item on the cursor (unmeasured). The research doc's `## Results` header carries `Logged shape` and `Supplied shape` columns and its `## Live procedure` the recording rule's `shape not reproduced` and `not-run (instrument` labels. On comment-stripped source it also pins `craftprobe`'s phase 0 additions (research build only): the three rows (`CreateItemNew`, `UiCreate`, `NetworkSendInventoryUpdate`) by SDK constant between the Phase 1k rows and the control, 285 rows, absent from `strip_research_blocks`; `CpResolveOther` defined once and called from `CpCall` and `CpCallMethod` before their dispatch; both dispatchers passing a separate other to `script_execute`; `CpMethods` free of `script_execute`, `CallBuiltinEx`, `MmCreateHook`, `variable_instance_set` and `variable_struct_set`; `callm`'s `inst` holder refused before the dispatch unless `CpRowForMethod` names a row; `call`'s closure refusal naming `callm` and `methods`; `CpResolve` reading `g_Orig_CreateItemNew` only under the `gml_Script_CreateItemNew` constant and before its one executable-code gate; `CpInstall`'s held check for an inline-detoured `CreateItemNew` deciding by that address (no flag in its condition) before `CpResolve`; the detoured line's `under table-only` marker; and `InstallItemInspectHooks` still installing `CreateItemNew` through `HookOneScriptTable`. Its check that the stash doc's `## Static search` names every candidate reads `CHARACTER_SELECT_OBJECTS | STASH_BAG_OBJECTS`, so the talent objects are checked against the skill doc instead. `MenuLayoutCellRows` pins the cell rows: `MenuLayoutCells(inst, true)` right after a grid's `Out(MenuLayoutRow(inst, sc))`, the `  cell=<x>,<y> grid=<id> fp=<fp|none> o=none` format with `o` never read, reads only through `variable_instance_exists`/`get`, `array_length`, `array_get` and `variable_struct_exists`/`get`, rows walked before columns, 200 per grid with `cellcap=1` on the grid's own row before `text=`, and no cell row counted toward the 200-instance cap. `StashBagPlayerVerbs` pins the five player verbs (`playerwarp`, `stashtab`, `bagtab`, `stashclose`, `giveitem`): each in `kPlayerCommands` (and `stashopen`/`stashmove` nowhere), each dispatched from its own `Handle*` helper as a standalone early return, `FrameCallback` naming none; their block free of any hook installer, `Rva`, `GetModuleHandle`, create, destroy, `event_perform` or `array_set`, no object name typed as a string; the tab and close routines exactly `UiACloseButton`, `UiAStashTabClick`, `UiAStashMaterialTabClick`, `UiAInventoryMaterialTabClick` and `UiAInventorySocketTabClick`, each an SDK constant paired with its own short name and dispatched through `TalentAllocDispatch` (`GetNamedRoutinePointer` before `asset_get_index` before `script_execute`); the block's one `CallBuiltinEx` Socketable's closure, called as the method value read from the button's own `activationFunc`; `giveitem`'s calls craftmats' own `kCm*` constants in `CmMakeUnit`'s order, its one struct write the save struct's `o` before `InitItemFromJson`, no `ChangeItemOwner`, `StashGridAddItem`, `RemoveItemFromMap` or `GridRemoveItem` in its body, the map-0 undo only after the add, and the stash destination and unmeasured sub-tabs refused `route_not_measured` before anything is called; each verb's lines tagged `<verb>: ` with a `before=`/`after=` line and a `refused - ` line; each shape live 2's supplied one; `playerwarp` writing `x` and `y` only.
  - `test_skill_actions_contract.py`: Pins the skill bar and talent tree research's round 0 (toolkit issue #147, `docs/skill-actions-research.md`) on comment-stripped source. `skillprobe` is research build only: every `Sp*`/`kSp*`/`g_Sp*`/`SKILLPROBE_*` symbol of its block is absent from `strip_research_blocks`, the word itself too, it is not in `kPlayerCommands`, and it is dispatched from `HandleSkillProbeCommand`, called from `RunCommand` directly after `HandleRestartProbeCommand` as a standalone early return (C1061), which answers false in the player build; `FrameCallback` never mentions it. Its row table is the thirty-three candidate scripts (the six constructor-style ones spelled as the SDK spells them, with no prefix; `CheckPlayerInteraction` last as the control; `KeyboardMouseInput` deliberately absent), the twenty-two scripts replan 3 added after live 1 found every closure that fired on a bind, an allocation or a reset logging `argc=0` (the fourteen `UiA*`/`UiSet*`/`UiHud*` activation scripts the talent buttons are wired to through `UiSetActivationFunc`, the five key getters, `DrawKeyBindSprites`, `LoadKeyboardControls` and `SaveControls`, each a `gml_Script_` constant, no mercenary name among them), and every SDK closure on the nine talent objects' `Create_0` - twenty-seven, derived from `scripts.hpp`, never from a hand list - eighty-two rows with distinct labels, each through its `HeroSiege::Scripts` constant's own value. `SpInstall` attaches each row by the block's one `MmCreateHook`, after `SpResolveName` (by name only) and behind `AddrIsExecutableInModule`, with no `HookOneScript(Table)`, `HookBuiltin`, `Rva` or module-base arithmetic; the detour calls the trampoline. A function another install already detours (`SpHolder`: craftprobe, tgprobe, prospectprobe, restartprobe, citrace nativetrace, mapkeep, craftmats, or a ForgePact hook that took the table entry inline, `SpKnownOriginal`) is counted `held` and skipped before anything is resolved, and `show` reads its count from the holder or prints `calls=n/a` on every `show`, never 0 and never nothing; a row held inline with no counter names its fix (`SpCountHint`: `tgprobe hook <Row>`, `tgprobe verbose on`, then `skillprobe hook <Row>`); `show` prints `CheckTalentUse` as the own-detour control beside `CheckPlayerInteraction` (`SpIsOwnControl`), and `INSTRUMENT-BLIND` when skillprobe does not detour it, because a held `CheckPlayerInteraction` proves only its holder's detours; the detour neither counts nor logs the instrument's own `ReturnTalentLevel` calls; the hook line is `<n> detoured, <f> failed, <h> held`. `call` is refused without the literal `confirm`, and every lookup (row, name, self, `other:` through craftprobe's `CpResolveOther`, each argument through `SpResolveArg`, which hands everything to `CpResolveArg` and is nothing but that delegation, so `skillprobe call` has no argument kind `craftprobe call` lacks - `id:` and `obj:` are `CpResolveArg`'s) comes between that gate and the one `CpDispatchScript`; every refusal says nothing was called; a closure row is refused naming `craftprobe methods` and `craftprobe callm ... inst <member>`, and `GetPlayerProfileObj` is never invoked. The block has no `script_execute`, `CallBuiltinEx` or `CallGameScriptEx` of its own, and its only other `CpDispatchScript` is `state`'s `ReturnTalentLevel` read, flagged as its own call so no detour logs it (`nolevel` skips it). `state`, `keys` and `slots` and their helpers mention no installer, write, event, create or destroy, and `keys` ends with one hint per key getter in `kSpKeyGetters` (each a row): arm it and read the HUD's own calls. Also: the nine talent objects in `kMenuLayoutObjects` (38 in `PLANNED_OBJECTS`), the bar's row followed by `MenuLayoutSlotRows` reading only through `variable_instance_get`, `array_length`, `array_get` and `variable_struct_get`, and the research doc's six headings, its `## Static search` naming every candidate (the replan-3 additions included), its `## Live procedure` carrying the recording rule's `shape not reproduced` and `not-run (instrument` and the capture's `## Checks` block, and exactly one line for each of its twelve `## Decision` keys, none `pending` since live 2 (`castKeyRule` and `pointsReader` beginning `not measured`, `bindRoute` and `resetRoute` `shape not reproduced`, the allocation, sub-allocation and screen-open routes `byname`). `menulayout`'s talent field list is `talentId` alone (the six other candidates pruned after live 2). `PlayerVerbs` pins the two player verbs: `skillstate` and `talentalloc` in `kPlayerCommands` and `skillbind`/`talentreset` nowhere (no `HandleSkillBind`/`HandleTalentReset`), each dispatched from its own `Handle*` helper as a standalone early return, `FrameCallback` naming neither; their block free of any hook installer, `Rva`, `GetModuleHandle`, create, destroy or `_set` builtin, every `CallBuiltin` a read, the one `CallBuiltinEx` the dispatcher's `script_execute` after `GetNamedRoutinePointer` resolved the SDK constant; `talentalloc` running exactly `UiAOpenTalents`, `UiATalentScreenTalent`, `UiAActivateSkillSpecialization` and `UiAActivateSkillSubPoint` (each constant paired with its own short name) and never `ReturnTalentLevel`/`ReturnSubTalentLevel` or a member named like a point count, printing `before=`/`after=` and `talentalloc:`-tagged refusals, the already-learned refusal before anything is called; `skillstate` reading only, its `effect=` an `instance_number` of an object resolved through `kSkillTimerNames`, and `SpHud`/`SpAbilityOf`/`SpSubText` delegating to its readers so `skillprobe state` prints the same body lines.
  ...
  - `test_craft_mats_behavior.py` + `craft_mats_harness.cpp`: **Behavioral** suite for the crafting-materials core (issue #14, Phase 0). Splices the real `plugin/include/ForgePact/CraftMatsMod.hpp` in at `// PRODUCTION_CRAFTMATS`. Baseline scenarios (`baseline/*`: off by default, off never plans a take, a bag that covers the need takes nothing - with the unreadable rule's negative control), target scenarios (`target/*`: only the shortfall is taken, from the stash tab only, never more than it holds; duplicate rows of one material summed, disagreeing reads refused; an unreadable count refuses and is named once; a declined take leaves the mod on; each loss signal turns it off for the session; the first-take and stat lines name work done). Each of six in-memory mutants of the header (bag ignored, no stash cap, loss keeps the mod on, unreadable read as 0, off still plans, never takes) failed at least one target or baseline on 2026-09-22. **The player build** (2026-09-24) adds `baseline/count_off_or_outside_the_route_is_the_games_own` and `baseline/press_off_is_vanilla`, and targets for the count adding the special tabs' stash, an unreadable stash leaving the game's count (named once), one display walk per game frame against a fresh press walk, the needs pairing each count with the decode before it, needs with no decode unreadable, the whole-then-partial split, a move confirmed only when both sides moved by the amount (nine loss shapes, the save-crash half state among them), the press gate and the craft only when every take is confirmed, the consume mismatch turning the mod off, and the save only after a confirmed move. They were written first and did not compile against the Phase 0/1e core; the first error lines are recorded in the harness comment.
  - `test_craft_mats_contract.py`: Pins issue #14's Phase 0 on comment-stripped source: `craftprobe` is research build only (every `Cp*` symbol stripped), not a player command, dispatched from `HandleCraftCommand` after `HandleMenuLayoutCommand`; its table names every candidate of the research doc's static search through `HeroSiege::Scripts` constants and covers every SDK closure of the eight cube/stash objects (a scan that must find at least 29); `CpResolve` refuses an address outside `Hero_Siege.exe` before `MmCreateHook`; the one write (`call ... confirm`) checks every precondition after the confirm gate and before its single `ApCallScript`, and each refusal says nothing was called; the readers never hook or write; `craftmats` is off by default, the command only flips the core's switch, keeps `stat` research-only and puts nothing on the frame path without the `IsEnabled()` gate (since the player build, only its install, once); the core header is game-independent, its source enum the two special tabs; the research doc carries its sections, status tokens and a procedure that runs the vanilla check and the hook-free readers before the hook. **Phase 1b** raised the table to 202 rows (the widened search's anchors named per family) and the closure scan to 81 over seventeen objects, and added `test_craftprobe_reports_its_phase_marker` (a bare `craftprobe`'s first line carries `phase1b rows=`), `test_craftprobe_var_follows_instance_refs_by_name_only` (a ref is classified from the runtime's text, `instance_exists` comes before any read of the instance, `ds_exists` before any `ds_grid_`/`ds_list_`/`ds_map_` read, a depth cap and a visited set, no `m_Pointer`/`m_Object`, no method call), `test_craftprobe_node_reader_is_hook_free_and_calls_only_the_fingerprint_lookup` (one `ApItemFromFingerprint` call, capped, the same helper `call`'s `fp:` uses), `test_craftprobe_node_reader_counts_the_items_o_member` (the save's one-letter stack key `o` is a candidate by exact name, never by substring; a definition struct with no candidate lists its numeric members, capped; a lookup miss names its `self` and `a1=0`; the research doc's `node-bag-control` row needs a sum equal to a bag stack seen by eye) and `test_craftprobe_backing_keeps_getter_returns_per_first_argument` (8 signatures for `GetInventoryArray`/`CountInventoryItem`, one dump file each). **Phase 1c** raised the table to 223 rows and the closure scan to 97 over nineteen objects (`Console_Save_obj`, `Profile_Manager_obj` added), retargeted the marker test to `phase1c rows=`, read the node tests over every `CpNode*` function, and added `test_craftprobe_node_lookup_takes_a_selectable_self_and_second_argument` (the research-only `ApItemFromFingerprintAs(self, fp, a1, item)` is the one script the node code calls; `a1=`/`self=id:`/`class=` parsed, a named self only after `instance_exists`; the instrument's own lookups flagged so a detour neither logs nor keeps them; a cap of at least 160; `node socket` reads each cell after `instance_exists`, once), `test_craftprobe_backing_keeps_fingerprint_lookups_per_second_argument` (`GetItemFromFingerprint` keyed on argument 1, `GetItemMap` on 0, both default rows; per signature a call count, the latest `a0` and six self objects at most), `test_craftprobe_store_reader_is_hook_free_and_names_its_objects_through_the_sdk` (the seven holders through `HeroSiege::Objects`, `instance_number`/`instance_exists` before any read, `ds_exists` through `CpDsText`, 400 names, `__cp_*` skipped, no write) and `test_craftprobe_node_sums_an_array_or_map_container_the_same_way` (`node var` resolves like `var`, reads a `ds_list`/`ds_map` only after `ds_exists`, reads an item struct's `b` and `o` directly, and ends in the same lookup pass and sum lines). The save-side count those readers are compared with is the hub's `tools/stash_tab_counts.py`, pinned by the hub's `tests/test_stash_tab_counts.py`. **Phase 1j** raised the table to 278 rows, retargeted the marker test to `phase1j rows=`, widened the one-write test to `call`'s and `callm`'s two `script_execute` dispatches and `set`'s one `variable_struct_set`, and added `test_craftprobe_phase1j_rows_are_sdk_declared_and_research_only`, `test_craftprobe_callm_invokes_a_method_value_by_name_behind_confirm`, `test_craftprobe_set_writes_one_existing_number_member_behind_confirm` and `test_craftprobe_inject_scopes_the_count_to_the_craft_route`. **Phase 1k** raised the table to 282 rows (three loader rows and `ReportClient`), retargeted the marker test to `phase1k rows=`, moved the Phase 1j rows' "just before the control" pin to "before the Phase 1k rows", scoped the Phase 1j doc slices to end at their Phase 1k counterparts, widened `test_research_doc_has_its_sections_and_status` with the Phase 1k sections and eighteen checks, and added `test_craftprobe_phase1k_rows_are_sdk_declared_and_research_only`, `test_craftprobe_callm_bind_rebinds_through_the_runtime_method_builtin`, `test_craftprobe_set_accepts_a_kept_return` and `test_craftprobe_call_keeps_its_own_return_for_kept` (`call` keeps its own dispatch's return in a slot no detour writes, rooted first; `kept:` reads it before the game's kept return, through one reader that names its cause when neither is there). **The player build** (2026-09-24) revises `test_craftmats_off_by_default_and_frame_path_gated` (the install is the frame path's one craftmats call, behind `IsEnabled()`, and the command still installs nothing), `test_core_header_is_game_independent` (the two-member source enum), `test_craftprobe_absent_from_player_commands` (`craftmats` is the one crafting player command) and the Phase 1h/1j row pins (the rows stay research-only; `PilipaliDecrypt`, `CreateItemSaveStruct` and `SaveLocalFile` reach the player build only through craftmats' own SDK constants), and adds `test_craftmats_is_a_player_command_and_stat_stays_research_only`, `test_craftmats_hooks_install_through_hookonescript_by_sdk_constant`, `test_craftmats_never_detours_a_script_craftprobe_holds`, `test_craftmats_count_hook_changes_its_return_only_in_route_and_on`, `test_craftmats_decode_hook_records_only_inside_craftfindrecipeitems`, `test_craftmats_stash_count_reads_only_the_two_special_tabs`, `test_craftmats_by_name_calls_are_the_recorded_shapes` (the set of by-name call shapes equals the measured list), `test_craftmats_refuses_before_calling_the_trampoline_when_a_move_is_unconfirmed`, `test_craftmats_save_follows_a_confirmed_move_only`, the panel's `test_panel_toggle_defaults_off_and_emits_craftmats` and `test_panel_text_is_player_facing_and_short`, and `test_research_doc_has_its_ship_design_and_phase_c_sections`.
  ...
  - `test_release_hook_contract.py`: Validates zero eager gameplay hooks in release builds, all-off pass-through behavior, and telemetry exclusions. The all-off test also enforces the owner's rule that **no mod is on by default** (2026-09-25): a default config sends the plugin no mod commands, and the test carries no exceptions. A new mod's `DEFAULTS` entry, its `cfg.get` fallback and its panel state all start off.
  ...
  - `test_incarnation_gems_behavior.py` + `incarnation_gems_harness.cpp`: **Behavioral** suite for the gem mod: the real `IncarnationGemsMod.hpp` against measured seeds. Baseline scenarios (switch off, a loaded gem, other items and socketables untouched) and targets (the Mythic seed for a drop's `n`, the filter's best match, its ties and misses, the dress, the tables' learning, resume and round trip, damaged or foreign tables starting empty). `INCARNATION_GEMS_BASELINE=1` stubs the two decisions; the red-first test pins that every drop, filter and dress target then fails.
  - `test_incarnation_gems_contract.py`: Pins the gem mod's wiring: the one native `DropGems` hook by SDK constant, installed once a player exists; the seed swap before the outermost `CreateItemNew` and only inside the drop scope; the dress on the finished item before Item Truth records it; the table candidates skipping every other post-process; the frame budgets; the three player commands with the research verbs kept out; the mod state; the panel's two switches (off by default, pinned by `test_both_switches_are_off_by_default`) and the filter (its list equal to the research doc's affix table, `/api/set` validation, the launch and live commands, the grouped Mods card).
...
- `docs/`: Reverse-engineering research logs, memory audits, and drop rate analysis.
  ...
  - `angelic-roll-hook-research.md`: Issue #64 - which step of the game's own Angelic roll picks the unique, so a follow-up can substitute Headhunter or Tyrant's Crown at Liquor Holster's share (Known Limitations item 21, the vanilla-roll gap). **Measured in live session 1 (2026-09-23, research build 18d2800, both controls passed): `finding: angelic-chance`** - `DropItemAngelicChance` ran once per game roll on its own detour, with the gate opened (181 rolls) and under a real buff 332 with the gate closed (193), always inside `DropItem`, four arguments with the chance third, undefined returned on every miss, and about eight `GetUniqueRepoStruct` reads inside each roll. `DropItem`, `GetUniqueRepoStruct`, `CreateDefaultParams` and `LootGroundCreate` are labelled `works` (the last two on ordinary drops only; the old "`LootGroundCreate` never called" zero was the table-only instrument), every other row `not observed`. Not observed: a roll hit (so a hit's return and its placement script are unknown), `lootListUnique` as a `Loot_Manager_obj` variable, and the 3000 that `buffme 332 3000 3000 18000` supplied as the chance (it stayed 1195/1526). Records the static search (seventeen candidate scripts and the names set aside), the per-route positive controls (C1: the kill hook, `angelic-chance` and `drop-item` with the gate open; C2: `angelicdrop`'s own placements through `LootGroundCreateFromItem`), `angelicprobe`'s three attach routes and its two attribution depths, the research build's startup gate lookup, the live procedure `L-1.1`..`L-1.13`, and what the follow-up workorder needs from it. Written to the strict standard (on `STRICT_DOCS`).
  ...
  - `crafting-materials-research.md`: ForgePact issue #14 (crafting at the game's Crafting Cube should use materials held in the shared stash's special tabs - Socketable, tab `-2`, and Materials, tab `-4`; both named by the owner during Phase 1). **Phase 1 done 2026-09-22 (`phase1-status: complete`), Phase 1b done 2026-09-23 (`phase1b-status: complete`), Phase 1c done 2026-09-23 (`phase1c-status: complete`)**: Phase 1 measured the vanilla baseline (not craftable from a special tab; the stash and cube windows cannot be open together), the cube's craft route and a **vanilla duplication** (a second craft with no inputs left still produced the result - recorded as a design constraint, `### Constraints from Phase 1`: the mod counts inputs itself and never extends the cube's own availability count), but reached neither the special-tab container (instance refs were not followed) nor the hand move's route (none of the 97 rows fired while the control climbed). Phase 1b widens the static search to 202 rows, teaches `var` to follow refs and adds the `node` reader and per-argument `backing`, with the hub's `tools/stash_tab_counts.py` as the save-side cross-check, and fixes the Live 2 check names under `### Live procedure 1b` and `### Phase 1b results`. Phase 1c is a reader round with no hand moves and no craft: 21 more rows (223) aimed at where `LoadStash` puts the stash while its window is closed, a `node` lookup whose self and second argument are selectable, `node socket` and `node var`, per-`a1` `backing` of the fingerprint lookup and the `store` reader, recorded under `### Phase 1c rows`, `### Phase 1c readers`, `### Live procedure 1c` and `### Phase 1c results`. Its session (Live 1c) read both special tabs' counts with the stash window open - the game's own `GetItemFromFingerprint` resolves a stash cell with `9` as its second argument, and `0`, the bag's value, resolves none - but no reader reproduced a special-tab count with the window closed, and in Live 1c the one reopen and the one tab switch each left new window instances; the `## Decision gate` reading was rewritten after it (`decision: pending`, still no hypothesis eligible, not a block). Phase 1d (done 2026-09-23, `phase1d-status: complete`) was a reader round on the Phase 1c build with no rebuild, adding `### Live procedure 1d` and `### Phase 1d results`: its session (Live 1d) read both special tabs' counts with the stash window closed from the map the game's own `GetItemMap(9)` call returned, kept by the instrument's detour and read through the same reference open and closed with the item-struct path's control passed, found `New_Inventory_Data_obj`'s `inventorySocketGrid`/`inventoryMaterialGrid` to be the bag's special tabs, and did not observe that map before the stash's first open in a launch or whether it follows a take (nothing was moved); the `## Decision gate` now reads H-A eligible, with what a player build would still need (the map without a blind getter call, a consume route that answers success, the duplication constraint). The owner then set `decision: H-A` (2026-09-23, "H-A, research consume first"), and Phase 1e (done 2026-09-23, `phase1e-status: complete`) was that consume-research round: a research build with `mapkeep` - the stash map kept through `HookOneScript`, the player-build installer, with a currency rule (`CraftMatsKeptMap`: current only when the game's own `GetItemMap(9)` returned it since the last character load or room change) - 29 more `craftprobe` rows (252) aimed at a stash-side take, and `call` forms for one confirm-gated by-name take of one unit, recorded under `### Phase 1e rows`, `### Phase 1e instrument`, `### Live procedure 1e` and `### Phase 1e results`. Its session (Live 1e) found `HookOneScript` installing both routes on `GetItemMap`, the game's first `GetItemMap(9)` call at the stash's first open, the kept map read whole (1626 entries), the currency rule invalidating it on a room change and the game's call at the reopen making it current again, and the map following both hand takes; a confirm-gated by-name `GridRemoveItem` on the Materials tab's grid answered `true`, emptied the cell and the saved `stash.hss` lost the unit, but the kept map still carried it, so no take the kept map confirms is on record, and `SaveStash` was observed at each window close. The `## Decision gate` now lists what an H-A player build has and still lacks (a take the map confirms, the save's source, a save after a mod take - the owner's cube-bracket proposal is the leading candidate - and the map before the stash's first open) and recommends one more research round. Phase 1f (done 2026-09-23, `phase1f-status: complete`, the owner's choice of that round) ran on the installed Phase 1e build with no rebuild and no code change, adding `### Live procedure 1f` and `### Phase 1f results` (sixteen checks, two launches): what the kept map does when the game's own hand move takes a stack's last unit out of each special tab, a one-unit by-name take on the Materials tab that the kept map, the game's lookup and the next stash save are all read after (the stack-decrement shapes of a hand split and a craft recorded, not called), and the owner's Cube bracket as two confirm-gated trials - `SaveStash` by name in the shape the stash close logs, open and closed, and `GetItemMap(9)` by name with self `Console_Save_obj` before the stash is opened in a launch - and it rewords two Phase 1e results as measured (the save seen at each close with no other writer of `stash.hss` ruled out; the map's behaviour on the game's own whole-entry removal not observed). Its session (Live 1f, two launches) found the game's own hand move of a whole stack out of either special tab dropping the entry from the kept map on the same map (so the stale entry after `GridRemoveItem` is that call's doing - repeated, with the game's own lookup still finding the unit), recorded the stash close's `SaveStash` shape (self `Console_Save_obj`, no argument), saw a by-name `SaveStash` with the stash closed dispatch with no new write of `stash.hss`, and obtained the whole stash map by name with `GetItemMap(9)` before any stash open, its counts equal to the file; the owner then redirected the round and set the build design recorded at the top of the `## Decision gate` (at the cube, count from the map obtained by name, move the shortfall into the bag for the game to consume, then save the stash by name), whose one unmeasured step is a by-name move of a stash item with the stash window closed. **Phase 1g (done 2026-09-23, `phase1g-status: complete`)** was one research build - the Phase 1e build's 252 rows with the marker `phase1g rows=252`, `craftprobe call`'s `undefined` argument and `within=` on the nine craft-route rows, nothing else - and one session that measured, with the stash closed and the Cube open, that move by name (`InventoryGridCanAddToStack`/`InventoryGridAddToStack`, then `ChangeItemOwner`, confirmed only by the kept map dropping the entry on the same index), whether a by-name `SaveStash` then writes, the recipe's readable members, and which craft-route row encloses both the consume and the result's production (`### Phase 1g instrument`, `### Live procedure 1g`, `### Phase 1g results`). Its session (Live 1g, fifteen checks) found the consume and the result's placement inside one `DoCraftResult` call in a one-unit craft (the recipe offered no amount 2), and that move confirmed for a Materials entry the bag already stacks, with self `Console_Save_obj` for the lookup and every call; the by-name `SaveStash` printed `NOT dispatched` with the stash closed and open and no write was seen, the game crashed inside its own `SaveStash` at the next stash close (cause not established), and the recipe's input members and a move of an entry the bag has no stack for were not observed - so the "After Phase 1g" paragraph at the top of the `## Decision gate` reads a player build as blocked on a save after the move and on the recipe's shape. Phase 0 (done 2026-09-22, `phase0-status: complete`) laid down the interpretation and the owner's decisions (count and consume; bag first, then the stash tab for the shortfall only; the stash need not be open if its tab is readable closed), the complete static search (97 `craftprobe` rows, including all 29 SDK closures of the cube and stash objects, and the sourced negatives), the vanilla baseline B0 to measure first, hypotheses H-A (count+consume hook), H-B (pull the shortfall stash -> bag through the game's own click-move by name) and H-C (count only), the instrument, the owner-run live procedure, the `## Results` table (filled from Phase 1; every negative reads "not observed by <what>") and the owner's `## Decision gate` (`decision: pending`). The owner picked H-A after Phase 1d and set its build design during Phase 1f; a closed-stash move the kept map confirms is on record since Phase 1g, and nothing ships until a stash save after it and the recipe's input members are too, and a player build is designed from them. Phase 1j (`phase1j-status: complete`) adds `### Phase 1j rows`, `### Phase 1j instrument`, `### Live procedure 1j` and `### Phase 1j results` for one more research build (24 rows, 278 in all, plus `craftprobe callm`, `set` and `inject`) and one session (Live 1j, 2026-09-24, 21 checks: 18 pass, 1 fail, 2 not-observed) that proved the by-name save route through `SaveLocalFile`, rejected one `callm` shape for a partial-stack take (self `Console_Save_obj`; the control's struct self and `UI_Split_Stack_obj` other, a shape for Phase 1k to try) and also proved a whole-item take-and-return the owner has since ruled out as the build's route, proved the count injection reaching the game's own availability display, and proved a whole-item placement into the Crafting Cube's own input grid (`New_Inventory_Data_obj.craftGrid`) - five route tokens proposed under `## Decision gate`'s After Phase 1j paragraph, the owner's to confirm before a build plan is written. Phase 1k (`phase1k-status: complete`) adds `### Phase 1k rows`, `### Phase 1k instrument`, `### Live procedure 1k` and `### Phase 1k results` for one more research build (three loader rows and `ReportClient`, 282 in all, plus `callm`'s `bind`, `set`'s `kept:` form and `call` keeping its own return; the SHA-256 and commit are in `### Phase 1k results`) and one session (Live 1k, 2026-09-24, `### Phase 1k results`: eighteen checks, 16 pass, 1 fail, 1 not observed) that rejected the one bound-call shape tried and instead proved a by-name partial take on the inline branch - the split's own per-unit edit with an inline write plus `ItemCheckHash` where the destination already stacks the material, and a new unit made by the game's own loader route (`CreateItemSaveStruct` -> `InitItemFromJson` -> `AddItemToMap` -> `GridAddItem`, the `StructCopy` clone branch not run) where it does not, into the bag or the Crafting Cube's `craftGrid`, with the game's own hash acceptance of an edited or created item left not observed - proposing three route tokens (`partial-stacked: inline`, `partial-nostack: json`, `partial-cube: proven`), which the owner confirmed on 2026-09-24 ("Confirm all three"), choosing to build on the inline edit while that hash acceptance is unobserved ("Build on it"). The player build that follows has its own three sections after `## Decision gate`: `## Ship design` (the hooks, the count, the needs pairing, the press order, every call shape and refusal, what is not observed live, and the player DLL's sha256), `## Phase C live procedure` (fourteen checks, the step-by-step in the workorder) and `## Phase C results`, with `phaseC-status` in the frontmatter.
  ...
  - `stash-bag-layout-research.md`: How the town stash opens and closes, which listed row is each stash and bag tab, which variable records the tab on show, where each grid cell is and what it holds, and which of the game's own routines, called by name, moves a whole stack or one unit between the bag and the stash or creates a test's items - the mechanisms the hub's `hs-drive` stash, bag and give-item tools (toolkit issue #147) are written against. Records the static search (the sixteen stash/bag objects added to `menulayout`'s table, and the #14 crafting-materials research, in-tree since the branch merged `main`, as prior measurement), a local static reading in our own words (`## Static readings`: the stash's interaction handler, the tab, grid, split and close routines), the instrument (`menulayout` with the added objects, optional fields and `[a,b,...]` array format, beside `craftprobe` and `citrace`, and the positive controls: `Player_obj` listed in `Town_01_rm`, `craftprobe hook`/`show` live, and `stashTabSelected` -2 on the Socketable tab, in both a dump and `menulayout`'s own field), the phase 0 live procedure (`dll-hash`, `marker`, `control`, P0-1 to P0-10, research build; setup by name, one hand-back of two arming hand moves), its `## Results` and fifteen `## Decision` lines. Live 1 and live 2 (2026-09-25) settled all fifteen: the warp by name, the open by the interact key F (the one by-name open was followed by a crash, causality not established), the close by `UiACloseButton`, the stash tabs by their own handlers (`tabNumber`, `stashTabSelected`), the bag's Materials sub-tab by its own handler (`uiNodeCallstack`, `tabSelected` on the stash window - not `stashTabSelected`; the by-name replay read -4 after it with no read before, so the switch by name itself is live 3's to confirm; `activeNode` not observed to follow), cells by `nodeFingerprint` only, the give route on #14's prior measurement (bag only); the two move lines moved to `hs-drive-stash-move-research`. The player verbs and the hub's five tools are written against those lines; live 3 (player build) verifies them.
  - `skill-actions-research.md`: Which routines and state the skill bar, a cast, a binding to a bar slot, a talent point's allocation (and a sub-talent node's) and a reset go through, and which of them can be run by name - the mechanisms the hub's `hs-drive` skill tools (toolkit issue #147) are written against. Records the static search (the nine talent objects added to `menulayout`'s table and their twenty-seven SDK closures, thirty-three candidate scripts, and the negatives the toggle-skill research measured), the Ghidra readings in its own words (the input's check sits above `TalentUse`, so a by-name cast skips the mana and requirement gate; the allocation buttons hash the talent state, so a raw write is ruled out and allocation, binding and reset go through the game's own handlers by name), the instrument (`menulayout`'s slot rows, `skillprobe`, and the readers it leans on: `craftprobe find`, `tgprobe deep`/`talents`, `citrace dumpobj`), the live procedure (S1-S8, research build, one hand-back, sharing the stash research's launch), its `## Results` and twelve `## Decision` lines. Live 1 (2026-09-25) settled `castByNameRoute`, `slotRule` and `talentIdRule`, and showed the bind, allocation and reset handlers are named `UiA*` activation scripts rather than the buttons' closures, so the static search gained twenty-two scripts (those activation scripts and the key getters) and a second research session (`### Live procedure 2`) measured the rest. Live 2 (2026-09-25, in the stash research's second launch) settled the other nine: the talent screen opens by name (`UiAOpenTalents`), and an allocation and a sub-node allocation replay by name (`UiATalentScreenTalent`, `UiAActivateSkillSpecialization` then `UiAActivateSkillSubPoint`, the real button as self), confirmed by `global.mySkills` and the sub-talent map; the bind (`UiATalentChange` on a popup button that is gone before a replay) and the reset (its confirm chain dispatched with no effect) are `shape not reproduced`; `castKeyRule` and `pointsReader` are `not measured` (no reader for a slot's key or the points left). So the player build ships `skillstate` and `talentalloc` only, and the verification session, `### Live procedure 3`, runs on the player build.
...
0. **For a drop lever, model the change first (before step 1):**
   - A change to `droprate`, `dungeonkey`, the relic pre-roll or a drop slider in `KEYS` is first expressed against `hs_game_sdk.drop_roll_model` in the hub's `tests/test_drop_roll_model.py`: a baseline test for the vanilla roll and a target test for the new lever, with the lever written as an input transform in the test, not in the SDK. Update the transform and `LeverParityTests` together with the plugin, and run `py -3 -m unittest tests.test_drop_roll_model -v` from the hub root.
   - The model settles the arithmetic (what a chance below 1 does, whether a multiplier squares, whether a base is read at all) without a launch. It cannot settle whether a hook attaches, frame timing, value kinds or how a rate feels in play; those still need the live steps below. The mechanism and its limits: `docs/models/drop-roll-spec.md`; the workflow: `docs/agents/static-model-workflow.md`.
...
3. **Update Panel UI & Command Builder (`src/forgepact.py`):**
   ...
   - A new Mods-tab row goes in `itemsCard` only if it does something for a specific
     item the player forged in the Item Editor with that Mechanic - tell by whether
     its description begins "For an item (or amulet) forged with Mechanic: <name>".
     Every other Mods-tab row goes in `qolCard` ("Quality of Life"). A row with a
     child (like Map Reveal's pack pass or Auto-prospect's material move) also needs
     its child row added to `preparePanelUI`'s parent/child grouping list for the
     card it lands in. `test_mods_categories.py` (issue #12) turns this rule into a
     check. A new Mods card also needs a sub-tab button in `#modsSubtabs` whose
     `aria-controls` names it - `ModsSubtabMarkupTests` fails if the two sets
     (sub-tab buttons and Mods cards) disagree.
...
- `autoprospect bag 1` / `autoprospect bag 0`: Auto-prospect's sub-option (issue #9, Stage C; panel key `mod_auto_prospect_bag`, nested under `mod_auto_prospect` the way `map_reveal_packs` sits under `map_reveal`; **on by default** in the core and the panel, so `build_cmds` emits `autoprospect bag 0` after `autoprospect 1` only when it is off, and turning the parent on live restates it; `autoprospect 1|0` leaves it alone).
  ...
  - `menulayout` / `menulayout <ObjectName>` (**player build**, in `kPlayerCommands`, dispatched from `HandleMenuLayoutCommand`; read-only): lists every live instance of thirty-eight candidate menu, stash, bag and talent objects, with its window position, so a tool that drives the game (the hub's `hs-drive`) can click where the game says a button is, or refuse.
    ...
    - For each of thirty-eight `hs-game-sdk` object names (the UI node/panel/list-item roots, `UI_Button_obj`, `UI_Button_Small_obj`, `UI_Character_obj`, `UI_Create_Character_obj`, `UI_Main_Menu_obj` and five unparented save/menu objects; then, for the stash and bag, `New_Inventory_Data_obj`, `UI_Inventory_Parent_obj`, `UI_Stash_obj`, `UI_Inventory_obj`, `UI_Stash_Tab_Bar_Container_obj`, `UI_Button_Stash_Tab_obj`, `UI_Button_Inventory_Tab_obj`, `UI_Button_Inventory_Tab_Small_obj`, `UI_Button_Close_obj`, `UI_Inventory_Grid_obj`, `UI_Split_Stack_obj`, `UI_Stash_Dropdown_obj`, `UI_Stash_Socket_New_obj`, `UI_Inventory_Drag_obj`, and the room-space `Town_Stash_obj` and `Player_obj`; then, for the skill bar and talent tree, `UI_Hud_Talent_obj`, `UI_Talent_Screen_obj`, `UI_Talent_Button_obj`, `UI_Button_Talent_Player_obj`, `UI_Button_Subtalent_obj`, `UI_Talent_Screen_Allocate_obj`, `UI_Sub_Talents_obj`, `UI_Talent_Node_Tree_Parent_obj` and `UI_Button_Sub_Skill_obj` - the room-space two being those whose `gui=` is a room position and whose `win=` means nothing) it resolves the object by name (`asset_get_index`, confirmed by `object_get_name` round-tripping), walks `instance_number`/`instance_find`, skips an instance id already printed, and reads through the handle `instance_find` returns.
    - Output, exact bytes (the hub's parse contract, pinned by `tests/test_menu_layout_contract.py`): a header `menulayout: room=<Room> gui=<W>x<H> window=<W>x<H> fullscreen=<0|1> view=<x>,<y>,<w>,<h>`; one row per instance, two-space indent, `obj=<Obj> id=<id> gui=<x>,<y> win=<cx>,<cy> bbox=<l>,<t>,<r>,<b> visible=<0|1> sprite=<Sprite|none>`, then whichever of `label`, `name`, `slot`, `index`, `page`, `selected`, `uiNodeCallstack`, `activationArgs`, `enabled`, `tabNumber`, `tabType`, `stashTabSelected`, `tabSelected`, `nodeGridWidth`, `nodeGridHeight`, `gridScale`, `gridName` the instance carries (an array value as `[a,b,...]`, each element through the same formatter, the first 32 then `,...+N`, a nested array as `<array>`; a reference, struct or pointer by kind only, `<ref>`, `<object>` or `<ptr>`, any other non-scalar kind as `<kind N>`; `activationFunc` is never printed), then always last `text=<to end of line>` (empty when it has none); and a footer `menulayout: listed=<n> absent=<names|none> capped=<0|1>` (200 rows at most; a name that does not resolve is listed in `absent=`). After the stash and bag fields a row prints `talentId` when the instance carries it (a list of its own, so the stash list keeps its pinned order; live 2 of the skill research printed it on the talent buttons, the sub-skill buttons and the sub-panel and none of the six other candidates - `subTalentId`, `talentLevel`, `treeIndex`, `slotIndex`, `allocated`, `pointsAvailable` - on any row, so those were pruned). A `UI_Hud_Talent_obj` row is followed by one `  slot=<row>,<i> talent=<talentId|none> gui=<navBboxX>,<navBboxY> win=<cx>,<cy>` row per element of its `row0` and `row1` arrays (32 per array at most; `none` for a field the element does not carry, `<read-failed>` for a read that threw; one `  slot=<row>,* absent` row when the instance has no such array and `  slot=<row>,* empty` when it has none in it), read only through `variable_instance_get`, `array_length`, `array_get` and `variable_struct_get` and scaled to the window as a row's `win=` is; which array is the drawn bar and whether `navBbox` is the point to click are `slotRule`'s to confirm (`docs/skill-actions-research.md`).
    - A `UI_Inventory_Grid_obj` row is followed by one `  cell=<x>,<y> grid=<instance id> fp=<nodeFingerprint|none> o=none` row per occupied node of its `nodeGrid` (an array of rows of cells; a node struct carries `nodeStartX`, `nodeStartY`, `nodeLocked`, `nodeIsPermanent` and `nodeFingerprint` and no count, so `o` is always `none` - `cellRule`, live 2 P2-5), walked row by row, read only through `variable_instance_exists`/`get`, `array_length`, `array_get` and `variable_struct_exists`/`get`; an item covering several cells prints one row per cell. Cell rows do not count toward the 200-row cap; a grid with more than 200 occupied nodes prints the first 200 and its own row carries `cellcap=1` before `text=`. The hub matches a cell by `fp=` (`itemRule: fingerprint`). With no window open only the HUD belt's `PotionGrid` grid exists, so no bag cell is listed then.
    - Two `win=` limitations, both why every stash and bag tab switch goes by name: a child of a scrolled container reports its own `win=`, not the clipped one - the stash tab strip's rows do not land where the listing says (live 1 P0-5); and a bag sub-tab row's (`UI_Button_Inventory_Tab_Small_obj`) listed `win=` lands on the page tab above it (live 2 P2-4; the operator's empirical offset under 1920x1080 is a session note, never a route).
  ...
  - `skillstate` (**player build**, in `kPlayerCommands`, dispatched from `HandleSkillStateCommand` as a standalone early return after `HandleSkillProbeCommand`; read-only, tool-facing): the skill bar and learned talents for the hub's `hs_skills_status`, `hs_skill_cast` and `hs_talent_allocate` (toolkit issue #147, `docs/skill-actions-research.md` § Decision).
  ...
  - The stash and bag verbs `playerwarp`, `stashtab`, `bagtab`, `stashclose` and `giveitem` (**player build**, each in `kPlayerCommands`, each dispatched from its own `Handle*` helper right after `talentalloc`'s as a standalone early return; tool-facing, they change game state - Known Limitations item 28): what the hub's `hs_stash_open`, `hs_stash_tab`, `hs_bag_tab`, `hs_stash_close` and `hs_give_item` send, each in the shape live 2 supplied and the game accepted (`docs/stash-bag-layout-research.md` § Decision). Every instance by `asset_get_index`/`instance_number`/`instance_find` (the player by `HhResolveLocalPlayer`), every routine by its `hs-game-sdk` constant (through `TalentAllocDispatch`: `GetNamedRoutinePointer`, then `asset_get_index`, then `script_execute` with self and other apart); nothing hooked, nothing on the frame path. Every line starts `<verb>: `; each prints one `before=`/`after=` line from its own re-read, and a refusal says what was not called.
    ...
    - `bagtab materials|socket` (`bagTabRoute`): `UiAInventoryMaterialTabClick`/`UiAInventorySocketTabClick` with self the `UI_Button_Inventory_Tab_Small_obj` whose `uiNodeCallstack` is `InventoryTabMaterial`/`InventoryTabSocket`, other the `UI_Stash_obj`, no argument; any other name is refused `route_not_measured`, and so is the bag without the stash open (`bag not open`). Prints `bagtab: before=<tabSelected> after=<tabSelected> activeNode_before=<id> activeNode_after=<id>` - `tabSelected` on the stash window is the state it proves (`bagTabState`, not `stashTabSelected`); `activeNode` is printed because live 2 did not observe it follow a by-name call. Refuses an unchanged state.
    - `giveitem bag <template fingerprint> <count>` (`giveItemRoute: bag: json`): the template found in map 0 by `GetItemMap(0)` and the runtime's `ds_map` builtins; then craftmats' own route and constants in `CmMakeUnit`'s order, self `Console_Save_obj`: `CreateItemSaveStruct(<template>)`, `o` set to the count on that struct (a template whose struct has no `o` - a non-stackable - takes count 1 and no `o`; a stackable takes at most the template's own `o`, since no stack-size reader is measured), `LootTimestamp`, the key text `0-0-<stamp>-<class>`, `InitItemFromJson`, `AddItemToMap` on map 0, `GetItemPreferredGrid(1, item)`'s `grid`, `GridAddItem` into it. A step that answers false or undefined stops there and is named; a unit no grid took is taken out of map 0 again (craftmats' undo). The proof is its own re-read at the point of use: `giveitem: key=<new key> before=<items> after=<items> o=<count|none>` (items in the destination cells, each counted once however many cells it covers), then `giveitem: confirmed - <key> in map 0 and in the destination cells` or `giveitem: not confirmed - ...`. `giveitem stash ...` is refused `route_not_measured` (the stash destination is `hs-drive-stash-move-research`'s); nothing edits an item after it is placed. Live 3's V0 confirmed the bag route placed a stackable material at count 1 in the same session (not dragged, saved or reloaded, and the session's save backup was restored afterward); the same session's V0b tried a non-stackable template (class 18) and it was refused `give_refused` - the verb's reader `ApPreferredGrid` found no array `grid` in what `GetItemPreferredGrid(1, item)` returned, or the call failed - so only the stackable-material case is confirmed placed.
    - Not built: `stashopen` (the by-name open crashed the game once in live 2; the hub presses F) and `stashmove` (the moves are `hs-drive-stash-move-research`'s).
    - `tests/test_stash_bag_layout_contract.py` (`StashBagPlayerVerbs`, `MenuLayoutCellRows`) pins all of this on comment-stripped source.
  ...
  - `angelicprobe on|show|reset|list` (**research build only**, not in `kPlayerCommands`, dispatched from `HandleAngelicProbeCommand` on the line after `HandleMenuProbeCommand`): the issue #64 instrument for which step of the game's own Angelic roll picks the unique; see `ForgePact/docs/angelic-roll-hook-research.md`.
    - `on` attaches seventeen candidate rows once (`DropItem`, `DropItemAngelicChance`, `DropItemAngelic`, the sibling drop entry points, the unique pickers, `CreateDefaultParams`, the three `LootGround*` placement scripts, `GetRareDropAnnouncement`), prints each row's route, and installs the kill control (`InstallHeadhunterHook`) so `kills=` counts, printing its route too.
    - **Three routes**, the `tgprobe` order: `detoured` (the script table still holds the game's function: the probe's own inline detour; the `angelic-chance` row reuses `raredrop angelic`'s `HookAngelicChance` and reports `HookOneScript`'s `nativeOut`), `detoured (under table-only <hook>)` (a ForgePact table-only hook holds the table - `LootGroundCreate`/`LootGroundCreateFromItem`, from `InstallItemInspectHooks` - so the probe detours that hook's saved original, which is still game code) and `via <hook> (native)` (DropManager's native hooks on `DropItem`, `DropItemBoss`, `DropItemAngelic`, `DropAngelicKey`, `DropAngelicCharm` hold the table from startup, so the research-only `BP_ANGELIC_PROBE_SCOPE` line in `FP_DROP_HOOK` counts the call). Anything else is `TABLE-ONLY (...)`, `blocked: ...` or `not found (...)`, and `show` prints that row as `calls=n/a`, never `0`.
    - `show`: per row the route, `calls=`, `insideDropItem=`, `insideAngelicChance=` (two thread-local depths: DropManager's `DropItem` hook body holds one, research-only lines in `HookAngelicChance` hold the other around every original call, extra rolls included), then `kills=`, the `DropItemAngelicChance` hook's totals, `lastChance=`, `lastReturn=<kind>:<value>` and the zero/nonzero/non-numeric return counts with the last nonzero return. The first three calls of each row are logged with argument kinds and `%g` values and the caller's object name. `reset` zeroes the counters and keeps the routes.
    - `list` reads `lootListUnique` off `Loot_Manager_obj`, resolved by name (`asset_get_index`, `instance_find`, `HhResolveInstance`), read-only: kind, length and the first eight entries' shape, or one refusal line.
    - Every detour target is a named table entry or a saved original a named install captured, checked with `AddrIsExecutableInModule` before `MmCreateHook`; the probe never calls a candidate (`DropItemAngelic` loops forever on an empty zone list). It runs inside `PollCommands()` like every command; nothing of it is on the per-frame path.
    - **Startup gate lookup.** Since ForgePact #69 (PR #75), both builds record `DropItem`'s and `DropItemAngelicChance`'s own code first thing in `InstallHook` (`CaptureAngelicScriptCode`; an entry is kept only when `AddrIsExecutableInModule` places it inside the game), and `FindAngelicGate` scans only that record, never the live script-table entry that `HookOneScript` swaps for ForgePact's own hook. So `raredrop angelic` works after `dropmult` in either build, which was verified live on 2026-09-23 after the failure was first reproduced on v1.4.5. The research `InstallHook` still calls `FindAngelicGate()` once before `InstallDropMultHooks()` for its log line (`angelic: gate found ...` or the reason it was not), but the order no longer decides whether the gate is found.
    - Session hygiene: a fresh session, and no `citrace nativetrace`, `raredrop ceiling`, `scount` or `zonegenlog` in it (each installs table hooks of its own), and no `angelicwatch` (its reset zeroes the kill and roll counters `show` measures from).
  ...
  - `gemmythic 1` / `gemmythic 0`, `gemfilter all` / `gemfilter <stat,...>`, `gemmaxroll 1` / `gemmaxroll 0` (+ `gems stat|save|reset|drop <n>|convert 1|0` in the research build only): Gems of Incarnation (ForgePact 1.4.6; player commands in `kPlayerCommands`; panel keys `mod_gem_mythic` and `mod_gem_maxroll`, both off by default, and `gem_filter`, `"all"` or a list of stat ids). The panel sends a narrowed filter at launch right after `gemmythic 1`, and again whenever Mythic is turned on.
  ...
  - `craftprobe hook [substr ...]` / `arm [budget=N] [inroute] [substr ...]` / `show [all]` / `reset` / `bag|stash|recipe [substr ...]` / `var <Obj|global> <nth> <name|a.b.c|*> [json]` / `var id:<n> <name|a.b.c|*> [json]` / `var <Obj> <nth>|id:<n> * from=<i>` / `find <Obj> <nth>|id:<n> [from=<i>] <text>` / `node id:<n>|<Obj> <nth>|stash|bag|socket [id:<n>]` / `node var <Obj|global> <nth> <a.b.c>` / `node var id:<n> <a.b.c>` (each `node` then takes any of `a1=<v>` `self=id:<n>` `class=<c>`) / `store [substr ...]` / `store names [substr ...]` / `backing on [substr ...]|off|dump|clear` / `dump` / `call <Row> <Obj> <nth>|id:<n> [other:<id>] [args ...] confirm` / `callm <Obj> <nth>|id:<n> <struct>|inst <member> [other:<id>] [args ...] [bind] confirm` / `methods <Obj> <nth>|id:<n>` / `set <struct> <member> <number> confirm` (struct also `kept:<row>[.a.b]`) / `inject <class> <b> <extra>|off` (**research build only**, not in `kPlayerCommands`, dispatched from `HandleCraftCommand`): the issue #14 instrument, Phase 1k build; see `ForgePact/docs/crafting-materials-research.md` § Instrument, its "Phase 1b additions", "Phase 1c readers", "Phase 1e instrument", "Phase 1g instrument", "Phase 1h instrument", "Phase 1i instrument", "Phase 1j instrument" and "Phase 1k instrument".
    ...
    - Phase 1g (the Phase A research build for the closed-stash move) adds no row: the marker becomes `phase1g`; `call` takes the literal `undefined` as a value of kind undefined (before, the word reached the game as a string), the second argument auto-prospect's move passes to `InventoryGridCanAddToStack` and `InvGridClearItemNode`, resolved after the `confirm` gate like every other form; and the armed line of the nine craft-route rows (`CraftFindRecipeItems`, `DoCraftResult`, `CraftEditGrid`, `CraftEditPlayerInventory`, `s_CraftItem`, `GridAddItem`, `GridAddToStack`, `s_ItemOperation`, `GetInventoryGridNode`) carries `within=<row>#<n>|none`, the outermost of those rows already on the stack when the call was entered and which call of it (`#n`, the number on that row's own entry and `ret=` lines, so two lines naming the same `#n` sat inside one frame and two different numbers inside two), tracked by the detours with a depth per row around the trampoline - the measurement of whether the consume and the result's production run inside one row a hook could refuse at.
    - Phase 1h (the Ghidra-read take, its save and the recipe's shape) adds two rows, 254 in all, both declared through their `hs-game-sdk` constants: `PilipaliDecrypt` (the decoder a recipe input's stored amount goes through when it is counted) and `CreateItemSaveStruct` (the per-item step of `SaveStash`, so a save's health reads per item). The marker becomes `phase1h`. `call`'s reply is split into four lines, each carrying the row's call number (`#<n>`, the number the row's own detour prints on its entry line for that call): `NOT dispatched #<n>: asset_get_index found no script`, `entered #<n>, script_execute threw`, `entered #<n>, script_execute returned st=<s>` and `dispatched #<n> -> ret=` - before, the first three were one merged `NOT dispatched` line, so a by-name call that faulted inside the game read the same as a name that never resolved. `var` and `path:` take a whole-number segment on an array (`is_array`, then inside `array_length`), so `path:Controller_obj.<S>.<k>` reaches a row of the stash's Socketable structure; on anything else the walk stops naming the segment. Its session (Live 1h, `phase1h-status: complete`, `### Phase 1h results`) measured a by-name `SaveStash` after a take that left the stash cell stopping with `entered #1, script_execute threw` (no write, the game running) at the point where the game's own save at the next stash close ended the game, but did not name `Controller_obj`'s stash containers - `var ... *` lists 80 of its 221 variables - so the stash-cell clear, the Socketable case and the selected recipe's decoded amount were not observed, and the next build needs a `var` listing past 80 and a way to tie a `PilipaliDecrypt` call to the selected recipe; the missing per-item fault line needs only a larger `arm budget=` (up to 5000 at run time), a procedure change rather than a build one.
    - Phase 1i (reach `Controller_obj`, finish the take, tie the recipe) adds no row, 254 still, and `kCpCraftRouteRows` keeps its nine labels: the marker becomes `phase1i`; `var <Obj> <nth>|id:<n> * from=<i>` lists 80 variables from the i-th name and a capped page names the exact next-page command and how many variables remain (the `bag|stash|recipe` readers and `store` keep "narrow the filter", since they take filters); `find <Obj> <nth>|id:<n> [from=<i>] <text>` is a hook-free, read-only content search printing the `var`/`path:` path of every string equal to `<text>` and every reference whose runtime text is it, walking arrays and plain structs but never instances, data structures or methods, with caps on depth (8), values visited (100,000, the cap naming the `from=` that resumes) and match lines (40); and `PilipaliDecrypt`'s and `CountInventoryItem`'s armed lines carry `within=` (the two named through `hs-game-sdk` in `kCpInRouteRows`, pushing no frame), while `arm ... inroute` logs them only while a craft-route row is on the stack, a call outside counting in `calls=` but spending no budget, so the selected recipe's decode at the press is not drowned by the Cube list's per-frame calls. Its session (Live 1i, `phase1i-status: complete`, `### Phase 1i results`: 17 of 19 checks pass, 2 not observed) named `Controller_obj`'s stash containers by content - `stashMaterialTab` and `stashSocketItemSlot`, each cell holding its fingerprint as `nodeFingerprint`, beside the map `stashInventoryMap` - ran the complete by-name take with its stash-cell clear (`GridRemoveItem`) on one Materials entry and one Socketable stack, after which the owner's stash close kept the game running and wrote a stash file holding neither, and read the selected recipe's amount (5, the owner's figure) from the `PilipaliDecrypt` call inside `CraftFindRecipeItems` at the craft press; a by-name `SaveStash` (self `Console_Save_obj` 0, no argument, stash closed, Cube open) returned cleanly after each take, one `CreateItemSaveStruct` call fewer each time, but wrote no file, the one before any take included, so the stash file's write rests on the game's own save at the next stash close.
    - Phase 1j (the save route, the partial take, the count injection and the Cube as a destination; `### Phase 1j instrument`) adds 24 rows, 278 in all, each through its `hs-game-sdk` constant - `SaveLocalFile`, `SaveStart`, `SaveCommit`, `SaveFileGMAsync` and `EncryptStringSave`, the item struct's ten methods, the creation candidates `s_ItemInstanceStruct`, `StructCopy`, `AddItemToMap`, `ItemCheckHash`, `LootTimestamp`, `GetCounterHash` and `EditItemData`, and `UiSetGrid`/`UiSetGridArray` - so `hook` expects `276 detoured, 0 failed, 2 held by mapkeep`, and three subcommands: `callm <Obj> <nth>|id:<n> <struct> <member> [args ...] confirm` invokes one method-valued member of a struct (`fp:<K>[.a.b]`, `fp9:<K>[.a.b]` or `path:`) by name through `script_execute`, self = other = the instance (`InvokeMethodValue`'s route A, no `CScriptRef` read), refused unless the member is a method; `set <struct> <member> <number> confirm` writes one existing number member and reads it back (`before=` / `after=`); and `inject <class> <b> <extra> [owner=<a0>]` / `inject off` makes `CountInventoryItem`'s detour return the game's count plus `<extra>` for that identity (owner 1 unless `owner=` says otherwise), only inside one of the game's three counting frames - the window's availability call `GetCraftItemsAvailable` (run by `UI_Craft_obj anon@1834`), the recipe row's closure (`UI_Craft_Recipe_List_Item_obj anon@840`), each with its own depth and neither a craft-route row, or a craft-route row - with `show` printing `inject: ... injected=<n>` split per frame plus the matching calls it left alone (`outside-route=`, `other-owner=` with the latest `a0`), so an unchanged display beside a count the injection did not reach reads not-observed rather than a fail; `call`'s argument forms moved into one resolver `callm` shares, unchanged. The marker becomes `phase1j`. Its session (Live 1j, 2026-09-24, `### Phase 1j results`: 18 of 21 checks pass, 1 fail, 2 not observed) proved the close's own by-name save route (`SaveLocalFile`, self `Console_Save_obj`, `a0=4`), rejected the one `callm` shape tried for the split's per-unit edit as a partial stash take (self `Console_Save_obj`; the control's struct self and `UI_Split_Stack_obj` other were not supplied, so this is a shape to revisit in Phase 1k, not a settled impossibility), also proved a whole-item take-and-return (`return-socket`), which the owner has since ruled out as the build's route, proved the count injection reaching `GetCraftItemsAvailable`'s own availability display (no craft press), and proved a whole-item placement into the Cube's own `New_Inventory_Data_obj.craftGrid` by name - whether the game counts that grid stayed not observed.
    - Phase 1k (the by-name partial take; `### Phase 1k instrument`) adds four rows, 282 in all, each through its `hs-game-sdk` constant - `InitItemFromJson`, `ReCreateItem` and `ParseItemToGrid`, the game's loader route that makes an item without its constructor, and `ReportClient`, so the hash check's "did not fire" is read from a detoured row - so `hook` expects `280 detoured, 0 failed, 2 held by mapkeep`; the marker becomes `phase1k`; `callm ... [args ...] bind confirm` re-binds the member to the struct it was read from through the runtime's own `method` builtin, by name, after every existing precondition and before the one `script_execute`, which gets the bound value (on the local Ghidra reading, Live 1j's four calls threw because the item's methods are stored unbound, so the instance ran as their self; `HashRouteMethod`'s shipped `+method` route is the precedent, never measured to change a hash), the reply adding `bind=yes` and `method_get_self` before and after (read only, unverified on this runtime); and `set kept:<row>[.a.b] <member> <number> confirm` writes into a row's kept return, refused naming what was supplied and why when that row keeps nothing. A `call` that ran now keeps its own return (`kept as kept:<row> #<n>`) in a slot no detour writes, rooted in `__cp_call_<row>`, and every `kept:<row>` reads that first and the game's latest `backing` return only when the row has none, printing which answered: the first Phase 1k build kept returns only from inside a craftprobe detour, so `kept:GetItemMap` could never fill (mapkeep, installed first, holds that row) and a game call could replace a kept value between the by-name call and its use. The slot is emptied before every dispatch, and a dispatch that did not return (no script, threw, a failed status) leaves it empty with the reason, so `kept:<row>` then refuses rather than handing on an earlier call's return or the game's (the Phase 1k build's second review round: after a thrown `call InitItemFromJson`, `kept:InitItemFromJson` would otherwise have re-added an earlier trial's unit); the kept line counts the row's by-name calls itself (`by-name call <k>`), since a mapkeep-held row's `#<n>` never moves. Without `bind`, `callm` is Live 1j's. One limit is recorded, not fixed: a `call` naming its own row's `kept:` argument passes a struct that is not rooted during that same dispatch, and nothing refuses the case - fixing it needs a rebuild, which would break the match between this source and the DLL Live 1k ran, and no Live 1k step named a row's own kept return. Its session (Live 1k, 2026-09-24, `### Phase 1k results`: eighteen checks, 16 pass, 1 fail, 1 not observed) rejected the one bound-call shape tried (`bind-control`: `method_get_self` showed the rebind applied, but the bound `script_execute` still threw and did not answer the count), proved the stacked partial take on the inline branch instead (`set itemDefinitionStruct.o` then `call ItemCheckHash`, both sides answering `bool:false`, the bag stack's `itemDataHash` measured changed and the stash entry's change supplied by the reading rather than itself observed), proved the no-stack take and the Cube placement through the game's own loader route (`CreateItemSaveStruct` -> `InitItemFromJson` -> `AddItemToMap` -> `GridAddItem`) into both the bag's `inventoryMaterialGrid` and the Cube's own `craftGrid`, and left the game's own hash acceptance of an edited or created item on a drag, a merge, a save or a reload not observed - proposing `partial-stacked: inline`, `partial-nostack: json` and `partial-cube: proven`, which the owner confirmed on 2026-09-24 ("Confirm all three") and chose to build on while the hash acceptance is unobserved ("Build on it"; `## Decision gate`'s After Phase 1k paragraph). The player build, `craftmats` below, rests on them.
    - Toolkit #147's stash and bag phase 0 (`ForgePact/docs/stash-bag-layout-research.md` § Instrument; `tests/test_stash_bag_layout_contract.py` pins it) adds three rows, 285 in all (`rows=285`, the marker word stays `phase1k`), each through its `hs-game-sdk` constant after the Phase 1k rows and before the `CheckPlayerInteraction` control, which stays last: `CreateItemNew` (the give-item route's own `InitItemFromJson` call is its positive control), `UiCreate` (the stash's open) and `NetworkSendInventoryUpdate` (on the grid handlers' path); a row whose function another install in this plugin already holds is reported `held by <install>`, neither detoured nor failed. The research build's own item-inspect hook swaps `CreateItemNew`'s table entry at setup (`bp_citemn`, table-only), so `CpResolve` detours the game function that hook saved as its original instead, through the same `AddrIsExecutableInModule` check (the `TgProbeAttach` shape), and the detoured line ends `(under table-only bp_citemn)`; when Custom Forge or Item Truth has already detoured `CreateItemNew` inline, its saved original is a trampoline and the row is `held by custom forge` / `held by item truth` - decided by that address, the flags only name the install. The summary then reads `held by mapkeep, craftmats or an item hook`. `call` and `callm` take `other:<id>` (after the self for `call`, after the member for `callm`): the instance passed as other, resolved like an `id:<n>` self (`instance_exists`, then by name) by one parser, `CpResolveOther`, and refused with nothing called when it does not resolve; both dispatchers pass it to `script_execute` separately, and without it the other is the self, as before. The reply then prints `self=` and `other=` in the armed lines' form, so a logged and a supplied shape compare field by field. `methods <Obj> <nth>|id:<n>` is hook-free and call-free: each variable of the instance, and each member of a plain-struct variable one level down, whose value the runtime calls a method, with the script it wraps (`method_get_index` + `script_get_name`) and the craftprobe row naming it. `callm`'s holder `inst` reads the member off the instance itself (`variable_instance_get`), for a closure a Create event stored there, and is refused before the dispatch unless a craftprobe row names that method's script; `call`'s refusal of a closure row now names this route (`methods`, then `callm ... inst` or `callm ... path:`). `crafting-materials-research.md` is #14's record and is not changed by these; `tests/test_craft_mats_contract.py` moved only the pins they invalidated (the row count, the Phase 1k rows' adjacency to the control, the dispatchers' `self, self` and the `self=other=` reply). Live 1 (2026-09-25) found that Custom Forge detours `CreateItemNew` at plugin start on every launch of this build, so that row is `held by custom forge` in every session (`282 detoured, 0 failed, 3 held` with `mapkeep on`), and that both by-name opens were `shape not reproduced`: no argument form could supply a live instance or an object reference. So the second launch's build adds two argument kinds to `CpResolveArg`, the one parser `call`, `callm` and `skillprobe call` share: `id:<n>`, that live instance's own `id` value (`instance_exists`, then `variable_instance_get(<n>, "id")` - a `ref instance N`, the kind a logged instance argument has; moved there from `skillprobe`'s `SpResolveArg`, which now only delegates), and `obj:<Name>`, what `asset_get_index` answers for that name, passed exactly as answered (`ref object <Name>` on this runner, the kind `UiCreate`'s logged first argument has) and refused unless `object_exists` and `object_get_name` say it is an object of that name. Each refuses before the dispatch with nothing called, and the reply prints each supplied argument through the value printer the armed lines use, so `ref instance N` and `ref object Name` compare field by field. It also adds one row, `UiACloseButton` (the close button's routine, for the by-name close), by its SDK constant after the three rows and before the control: 286 in all, `rows=286`; the restart probe's table names the same script, and whichever installs first holds it. `tests/test_craft_mats_contract.py` moved only its row-count pins for it (the fourth `STASH_BAG_ROWS` entry and the adjacency range).
  - `mapkeep on|off|stat|find <class> <b>|clear` (**research build only**, not in `kPlayerCommands`, dispatched from `HandleCraftCommand`): issue #14 Phase 1e's keeper of the stash map (`ForgePact/docs/crafting-materials-research.md` § "Phase 1e instrument"). `on` installs hooks on `GetItemMap` and `LoadStash` through `HookOneScript`, the player-build installer, and prints one line per hook, `both-routes` or `TABLE-ONLY (...)` - the answer to whether the player-build shape reaches the map; run it **before** `craftprobe hook` (it refuses when craftprobe already holds either row, since the installer would then report a false TABLE-ONLY). The hook bodies count `GetItemMap` calls by first argument (`a0=0`, the keeper's control; `a0=9`; other), keep the game's own `a0=9` return in the research global `__cp_mapkeep_9`, and invalidate it on `LoadStash`; a room change invalidates it too, noticed by the frame poll (every 30 frames) and on the keep path, which reads the room before deciding an `a0=9` return is already current - so a game refresh in a new room before the next poll counts as the refresh after the change (`room-changes=` grows, the keep line reads `was room-changed`). `stat` prints the installs, the counts, `kept=`, `current=yes|no` with `reason=` (`not-kept`, `character-loaded`, `room-changed`, `ds-gone`, `none`), `refreshed=` and `first9:` (the first `a0=9` call's number, self and room). `find <class> <b>` walks the kept map by name (4000 entries) and prints each matching key with its `o`. Currency is `CraftMatsKeptMap`'s rule, asked at the point of use: current only when the game's own `GetItemMap(9)` returned the map since the last character load or room change - `ds_exists` alone never makes it current, because GameMaker reuses map indices. Nothing here calls a game script.
    - `hook` native-detours all 252 candidate rows (resolved by name, refused outside `Hero_Siege.exe` before `MmCreateHook`) - Phase 1b added the grid-input/drag, add/stack, socket-family, stash-tab-buy and node/tab rows plus every Create closure of nine grid, inventory and stash objects; Phase 1c added `GetItemMap`, `GetInventoryMapPos`, `SaveInventoryMap`, `LoadInventoryOrderNew`, `s_SaveStashConstants` and every Create closure of `Console_Save_obj` (6) and `Profile_Manager_obj` (10), which `citrace` also names. `CheckPlayerInteraction` is the positive control; `arm`/`show` budget and report the per-call self/other/arguments/return lines.
    - The readers are hook-free and write nothing. `var` follows a live `ref instance N` by name: `object_get_name` of its `object_index`, its id and its variables, each further instance reference one level deeper (depth 2, 12 instances, a visited set), and only after `instance_exists` answers true; a `ref ds_grid|ds_list|ds_map` is read only after `ds_exists` with its own type answers true; `id:<n>` starts from an instance number, `a.b.c` walks a dotted path, `*` lists every variable; a method value is described, never called; no struct-layout read. `node` reads a container's `nodeGrid` (size, filled/empty, distinct `nodeFingerprint`s), makes one by-name `GetItemFromFingerprint(fp, a1)` per fingerprint (capped at 160 per command; the research-only `ApItemFromFingerprintAs` beside the player build's `ApItemFromFingerprint`, whose shape stays `(fp, 0)` with the grid as self), prints `itemType`, `itemDefinitionStruct.b`, the class suffix and every numeric member that may be the stack count - named exactly `o` (the save's stack key, which the item struct shares with `b`) or like `stack`/`amount`/`count`/`qty`, and, when the definition struct has none, all its numeric members (24 at most) - then a `sum class=<c> b=<b>` line per pair; a lookup miss prints what was supplied (`self=<object> id=<n>, a1=<a1>`); `stash` reads each live instance reference of the open stash window, `bag` each `UI_Inventory_Grid_obj` (the known-good control, which passes only when a sum equals a bag stack the owner states by eye - on the bag tab that is on show, since the bag's `InventoryGrid` held whichever tab was shown), and an instance without `nodeGrid` prints its variable names. Phase 1c options: `a1=<v>` (the lookup's second argument, default 0), `self=id:<n>` (its self, default the grid read; used only after `instance_exists` and a by-name resolve), `class=<c>` (look up that class suffix only; the rest are summed `b=?`). `node socket [id:<n>]` walks the Socketable window's (`UI_Stash_Socket_New_obj`) `grid` array of per-cell `UI_Inventory_Grid_obj` instances - each after `instance_exists`, once, 400 at most - into one sum table. `node var` sums whatever a `var` path reaches: a `nodeGrid` instance as `node id:`, or an array, `ds_list`, `ds_map` (after `ds_exists` with its type) or struct entry by entry - fingerprints looked up, item structs read directly (`b`, `def.o`), 2000 entries at most (1000 before Phase 1e). `store [substr ...]` lists, for the first live instance of `Console_Save_obj`, `Profile_Manager_obj`, `Town_Stash_obj`, `Player_obj`, `New_Inventory_Data_obj`, `Inventory_Loading_obj` and `Load_Inventory_obj`, the instance the kept `GetProfileInventoryData` return refers to, and the globals, every variable matching the filters (default `stash socket material item map inv tab`) with its shape, 80 per holder; `store names` prints the names only, ten per line, 400 per holder; the instrument's own `__cp_*` globals are skipped.
    - `backing` keeps what the game's own getter calls returned (never invokes one); per row it also keeps the latest return per distinct signature of one argument (8 signatures) - `GetItemFromFingerprint` on its second (`a1`), `GetItemMap`, `GetInventoryArray` and `CountInventoryItem` on their first - with a call count, the latest `a0` and up to 6 self objects per signature, and `backing dump` writes `cp_backing_<row>_arg<k>.json` per signature. `GetItemFromFingerprint` and `GetItemMap` are default `backing on` rows (the lookup runs ~20k times a minute while the bag or stash draws; `backing off` if the game stutters); `node`'s own lookups are neither logged nor kept. `call` is the one write - a single by-name `script_execute` of one plain-script row behind the literal `confirm`, refused before the call on any missing precondition.
    - Several rows share an address with `prospectprobe`'s table and with `autoprospect 1` (`UI_Inventory_Grid_obj anon@15345`): one instrument per session, Auto-prospect off in the panel. The save-side count it is compared with is the hub's `tools/stash_tab_counts.py` (`docs/tools/stash-tab-counts.md`).
  - `skillprobe hook [substr ...]` / `arm <Row>|all [n]` / `arm off` / `show [all]` / `reset` / `call <Row> <Obj> <nth>|id:<n> [other:<id>] [args ...] confirm` / `state [nolevel] [bind=<Obj|global|id:n>.<a.b.c> ...] [profile=<Obj|id:n>]` / `keys [<Obj|global|id:n>[.a.b.c] ...]` / `slots [<row> <i>]` (**research build only**, not in `kPlayerCommands`, dispatched from `HandleSkillProbeCommand` after `HandleRestartProbeCommand`): the skill bar and talent tree phase 0 instrument (toolkit issue #147, `ForgePact/docs/skill-actions-research.md` § Instrument; `tests/test_skill_actions_contract.py` pins it).
    - A bare `skillprobe` prints the usage, whose first line is the build's marker, `skillprobe: rows=82 hooked=<k> held=<h> - ...` (the build live 1 ran printed `rows=60`, before the replan-3 rows).
    - One table of eighty-two rows, each through its `hs-game-sdk` constant: the thirty-three candidate scripts (the cast - `CheckTalentUse`, `TalentUse`, `TalentUseClass`, `TalentRequirement(Func)`, `GetTalentCooldown`; the talent readers; the bar's and talent screen's activation and navigation constructors; the network and consistency updates; `LoadControls`/`SaveControlsFunc`; `GetPlayerTalentHudObj`, `GetPlayerProfileObj`, `PlayerManaUpdate`, `UiCreate`, `UiSetFocus`, `ReportClient`), the twenty-two scripts added after live 1 (the named activation scripts the talent screen's and the bar's buttons are wired to through `UiSetActivationFunc` - `UiATalentScreenTalent`, `UiATalentScreenAssign`, `UiATalentChange`, `UiAActivateSkillSubPoint`, `UiAActivateSkillSpecialization`, `UiATalentScreenResetTalents`, `UiAResetSubSkillPoints`, `UiATalentsPlayer`, `UiAOpenTalents`, `UiAActiveTalentSelect`, `UiAContextTalents`, `UiATalentScreenTalentLoadout`, `UiSetTalentSelectTopRowEnabled`, `UiHudTalentNavigation` - because every closure that fired on live 1's bind, allocation and reset logged `argc=0`; and the key getters the bar draws its key letters with, `GetSpecificKeyBind`, `GetSpecificKBKeyBind`, `GetSpecificGPKeyBind`, `GetPlayerInputBindings`, `GetControlName`, with `DrawKeyBindSprites`, `LoadKeyboardControls` and `SaveControls`) and the twenty-seven Create closures of the nine talent objects, with `CheckPlayerInteraction` last as the positive control. The six constructor-style scripts the SDK spells without `gml_Script_` are resolved by that name and then with the prefix; a row neither finds is counted `not found by name` on the hook line, apart from `failed`.
    - `hook` native-detours each row with `MmCreateHook` at the function's own address, resolved by name and refused unless `AddrIsExecutableInModule` says it is game code - craftprobe's attach. A function another install already detours is `held`, neither detoured nor failed, and never hooked twice: a `craftprobe`, `tgprobe`, `prospectprobe` or `restartprobe` row, a `citrace nativetrace` target, `mapkeep`, `craftmats`, or a ForgePact hook that took the table entry with an inline detour (`toggleguard`'s `HookTalentUseClass`); where such a hook is table-only its saved original is the game's function and is detoured instead (`under table-only`). The answer is `skillprobe hook: <n> detoured, <f> failed, <h> held.` So run `skillprobe hook` after the other instruments' `hook` and after `toggleguard 1`: they do not know this table, and one of them hooking second would fail on a row this one holds. In the launch shared with the stash research, craftprobe's `ReportClient`, `UiCreate`, `GetPlayerProfileObj` and `CheckPlayerInteraction` rows are expected `held`. A row held by an inline ForgePact hook has no counter and no argument log until tgprobe attaches it through that hook's entry note, and its `hook`/`arm`/`show` line says so: for `TalentUseClass` under `toggleguard 1`, run `tgprobe hook TalentUseClass` (that row only - an unfiltered `tgprobe hook` would take `CheckTalentUse`, the own-detour control, too), `tgprobe verbose on`, then `skillprobe hook TalentUseClass`, and the row reads `held by tgprobe hook` with a count.
    - `arm <Row> [n]` logs the next `n` calls (default 3, at most 500) of that row, `arm all [n]` every row but the control, `arm off` none; counts run regardless. A logged call prints `skillprobe <Row> #<n> self=<Obj#index@id> other=<Obj#index@id> argc=<k> a0=... ` in craftprobe's armed-line form, then `skillprobe <Row> #<n> ret=...`. `show [all]` prints each row's `calls=` since the previous `show`, the two controls first: `CheckPlayerInteraction`, and `CheckTalentUse` as the own-detour control (once per frame, measured), whose climbing count is what proves skillprobe's own detours when `CheckPlayerInteraction` is held by another probe - not detoured here, that line says `INSTRUMENT-BLIND`. A held row's count is its holder's (and says so); a row with neither a detour nor a holder's counter prints `calls=n/a (...)` on every `show`, never `0` and never nothing. `state`'s own `ReturnTalentLevel` calls are neither counted nor logged.
    - `call` is the one write: ONE by-name dispatch of one plain-script row, refused without the literal `confirm` and on any missing precondition before it (an unknown row, no resolved pointer, no such instance, an argument that does not resolve), each refusal saying nothing was called. It uses craftprobe's own `CpResolveOther` for `other:<id>` and craftprobe's `CpDispatchScript` for the dispatch; arguments are craftprobe's `call` forms, resolved by craftprobe's `CpResolveArg` - `id:<n>` among them, which passes that instance's own `id` reference (the kind a logged player-ref argument has), and `obj:<Name>`, an object reference. A closure row is refused, naming `craftprobe methods <Obj> <nth>` and `craftprobe callm <Obj> <nth> inst <member> [other:<id>] ... confirm`; `GetPlayerProfileObj` is refused as a profile getter. The reply prints self and other in the armed lines' form, the row's call number and which of the four dispatch outcomes it had (`NOT dispatched`, `script_execute threw`, `returned st=`, `dispatched -> ret=`).
    - `state`, `keys` and `slots` install no hook and write nothing. `state` prints, each read in its own `try` and `unreadable` rather than a default: every `row0`/`row1` element's `talentId` with its `abilityId` (from `global.talentStructMap`) and `refreshInfoTimer`; `global.mySkills`; the bar's `playerSlot.bind_skill`; each `bind=` path (craftprobe's `var` walk); `global.subTalentMap[1].t<id>` for each bar talent, every `s<NN>` with its level (`none` for a talent with no node); `ReturnTalentLevel(<id>)` by name for each bar talent, self the local player, run through `CpDispatchScript` as the instrument's own call so no detour logs it (`nolevel` skips it; its argument shape is phase 0's to confirm against an armed `ReturnTalentLevel` row); and the points candidates - every numeric member of the player, and of `profile=` if given, whose name contains `point`, `talent` or `skill`. `keys` prints every member of `Controller_obj` and of the bar's `playerSlot` (or of the roots given) whose name contains `skill`, `talent` or `bind`, arrays and structs three levels deep, then one line per key getter (`GetSpecificKeyBind`, `GetSpecificKBKeyBind`, `GetSpecificGPKeyBind`, `GetPlayerInputBindings`, `GetControlName`) saying to arm it and read the HUD's own calls, with whether its row is detoured yet - live 1 found no member holding a key per slot, while the HUD draws one, so the key is read from the getter's logged calls. `slots` prints `menulayout`'s own row and slot rows for the bar, then every member of one element (`row0[0]` by default).
...
6. **Diagnose stalls with the built-in watchdog, do not guess:**
   ...
   - **Measured 2026-09-23, Underground Garden freeze (#58): ours, through YYToolkit's console.** Entering Underground Garden from Misty Swamp froze on "Generating Zone..." for 30-70 s in the player build with every mod off, in every published version.
     - An out-of-process stack sampler (dbghelp `StackWalk64` on the main thread at 10 Hz, the thread suspended only while its stack is read) found the main thread in `ntdll!ZwWriteFile` for about 72 s, called from the game's own code, with no ForgePact, YYToolkit or other Aurie-module frame on those stacks.
     - The game had a `conhost.exe` child from its first second. Reading its console buffer (attach, read, detach; nothing written) showed all 9001 rows taken by one warning: `tilemap_get() - couldn't find specified tilemap`, from `Controller_obj`'s Alarm 1.
     - Cause: YYToolkit's `AllocConsole` makes its console the game's standard output, so every GameMaker runtime warning became a synchronous console write. Hiding the window (the old `KonsoluGizle`) did not stop the writes. The unmodded game has no console, so there they fail at once.
     - Fix (ForgePact PR #75): the player build calls `FreeConsole` (`DetachConsole`) instead. The same entry then loaded in 1-2 s and the sampler saw no `WriteFile`. The research build keeps its console, so it still freezes there.
     - Next time a freeze happens "only with mods", check for a console first: `AttachConsole` to the game's pid fails with error 6 when it has none, and succeeds (and lets you read the buffer) when it has one.
...
21. **Headhunter and Tyrant's Crown now drop through the Angelic pool instead of their own die (#63, 2026-09-22, owner-directed):**
    ...
    - **The vanilla-roll gap (accepted; a follow-up issue tracks it separately).** Liquor Holster can also drop from the game's own Angelic roll under an "Angelic item drop chance" effect (Blood Pact / dungeon modifier). Headhunter and Tyrant's Crown are not in the game's own unique loot list (`GetUniqueRepoStruct` has no entry for them; `SpawnSignatureItem` builds them through the forged-item path the Custom Forge uses instead), so that path can never drop them - only ForgePact's own die does. Magic Find's effect on the game's own roll was not measured either. Matching this would need a hook on the game's own Angelic roll; which step of that roll to hook is measured in `ForgePact/docs/angelic-roll-hook-research.md` (issue #64, research build only - `angelicprobe`). Its live session ran 2026-09-23 with both positive controls passing, and the doc's `finding:` line reads `angelic-chance`: `DropItemAngelicChance` is the step that runs once per game roll, on both ways of making the game roll, on a route that sees the game's direct call, with the pick inside it. Still not observed, and the follow-up has to plan around each: a roll hit (so neither a hit's return value nor the script that places its item is known), `lootListUnique` on `Loot_Manager_obj`, and the chance following the buff value `buffme` supplied. The substitution itself is a follow-up workorder planned from that line.
...
23. **Half of the Relics slider does nothing observable: `droprate group relic` divides every relic base by the same factor, and `DropRelic`'s drop is not a 1-in-base roll (recorded 2026-09-24, issue #162):**
    - **What is measured.** `DropRelic`'s drop is not a 1-in-`droprate.base` roll: all 156 relics carried 25,000,000 and relics still dropped constantly (measured 2026-08-28, ForgePact `c0a6a6b`; not re-measured since). `build_key_cmds` still sends `droprate group relic <m>` with every Relics setting, and the plugin's `relic` branch of `DropRateCmd` still divides all relic bases by the same factor, so that half of the slider writes values for which no reader was observed. Whether `DropRelic` reads the base some other way, for example as a weight in the pick between relics or when reached from one of its other direct callers, is not established: every relic had the same value, a uniform divide leaves such a pick unchanged, and no run changed a single relic's base. So do not take the field as dead when weighting relics per item. The slider's real effect is the other half: the type-41 gate through `dungeonkey`, thinned by the quadratic pre-roll (`g_DkTipOlcek`).
    - **Harmless, not removed.** The write is cheap and the relic bases are restored at x1. Removing the command is a separate ForgePact change; this entry only records it.
    - **Where it is checked.** The hub's `tests/test_drop_roll_model.py` (`TargetTests.test_m7_relic_base_lever_is_a_no_op`) shows the lever writing a relic base that the modelled relic roll cannot read, and `docs/models/drop-roll-spec.md` records the mechanism.
24. **"Craft from the stash" (`craftmats`, issue #14) moves the stash's materials and saves the stash at the craft press, a moment the game did not choose - an accepted risk (2026-09-24):**
    - **What it does.** With the switch on, a Crafting Cube recipe's count gains the stash's Materials and Socketable tabs, only inside the game's own crafting route (the availability call, the recipe row's Create closure, and `CraftFindRecipeItems` at the press); outside it every count is the game's own. At the press, only the amount the bag is short of moves from those tabs by the game's own routines - onto the bag's stack, into a new bag stack, or into the Cube's `craftGrid` - before the game's own `DoCraftResult`, and the stash is saved by the game's own route (`SaveLocalFile(4, 1)`) right after. Design and every call shape: `ForgePact/docs/crafting-materials-research.md` `## Ship design`.
    - **The accepted risk.** Items move, and the stash is saved, at a moment the game did not choose. Quoting the owner's answer to the build's risk question (2026-09-24): "accept". The game's own craft can produce with nothing consumed (the vanilla duplication, `### Constraints from Phase 1`), so the mod never lets the stash supply an input it did not count and move: a press whose moves cannot all be confirmed is refused before the game's call (`craftmats: ... the craft was refused`), and a craft whose consume does not match what moved turns the mod off for the session (`craftmats: consume mismatch - ...`). The count is injected only inside the crafting route.
    - **Two additions the owner accepted after they were explained (2026-09-24, "Accept").** The decode hook sits on a hot game function, `PilipaliDecrypt` (672,588 calls in one research session): outside the `CraftFindRecipeItems` frame its body only forwards, but it is on the game's path for as long as the hooks are installed, off or on. And after a press-time stash save the character's own save is left to the game: a crash between the two loses that craft's stash-supplied materials.
    - **What is not observed live** (the owner's choice of two Phase C cases, "Keep two cases"): the Cube fallback (a full bag), a whole-entry take (a shortfall that empties a stash stack), and the game's own hash acceptance of an edited or created item beyond Phase C's watch for a flagged item. The undo shapes a failed take runs (`RemoveItemFromMap` on map 0, `GridRemoveItem` on a bag or Cube grid, `AddItemToMap` on map 9) reuse measured scripts on another map or grid and were not run live.
...
- Shared game facts folded from the research docs below: `../../RUNTIME_DATA_MODELS.md` §5-§17 (§16 from Item Truth, §17 from issue #14) (a fact about the game itself goes there too, per `AGENTS.md` § "Fold What a Mod Learned About the Game Into the Shared References")
...
`plugin/include/ForgePact/IncarnationGemsMod.hpp` and the Gems of Incarnation
block of `plugin/ModuleMain.cpp` (after `TruthBuildItem`) make every Gem of
Incarnation that drops Mythic, with the mods the player ticked, and every gem's
affixes show their best tier's top. It came from a player's report on the
toolkit's Discord: most gems drop with one weak mod and the loot filter cannot
hide them.

Related documents:
- The game facts it established:
  [`RUNTIME_DATA_MODELS.md` §18](../../RUNTIME_DATA_MODELS.md#18-gems-of-incarnation).
- The research, the full tables and the live checks:
  `ForgePact/docs/incarnation-gems-research.md`.
- The commands and the log lines: "Command Reference" above.

How it works:
- **The swap happens before the roll.** The game builds a gem from its
  definition alone, so a drop is replaced by giving `CreateItemNew` another
  seed, one the game itself rolled Mythic at the same `n`. The mod never
  invents an `n` (it is a table index, not a scale).
- **`DropGems`, not `LootGroundCreate`.** `LootGroundCreate` is the natural
  site, but the Mining Ore mod already holds its native detour, and a second
  native detour of one script falls back to table-only, which compiled GML's
  direct calls bypass. `DropGems` scopes the swap instead: a thread-local depth
  counts its calls, and only a `CreateItemNew` inside it is a fresh drop.
- **The tables are the game's own rolls.** `GemsTick` builds candidate gems
  through `TruthBuildItem` (Item Truth's save-loader build) on a fixed
  per-build seed sequence, so a half-built table resumes. The candidates set a
  thread-local flag that makes the item hook return before Custom Forge, the
  dress and Item Truth. 512 Mythic seeds per `n`: at that size every mod of the
  pool is in each table, +All Skills (0.7% of Mythic gems) 3-4 times.
- **The filter picks, it never rejects.** Seeds are scored by how many ticked
  mods they carry; a drop takes one of the top-scoring seeds at random. Many
  ticked mods therefore give each gem as many of them as any seed has, and a
  rare mod may repeat the same few rolls.
...
- **Tests.**
  ...
  - `tests/test_incarnation_gems_contract.py`: the wiring, the panel and the
    filter.
- **Live result, 2026-09-25, research build, build `pe-6aaa6779-0cad4fc8`:**
  - the tables for `n` 3, 4 and none filled with 512 Mythic seeds each in about
    45 s at the main menu (47,147 candidates, 37 best ranges, no conflicts);
