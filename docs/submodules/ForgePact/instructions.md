...
- `docs/`: Reverse-engineering research logs, memory audits, and drop rate analysis.
  ...
  - `crafting-materials-research.md`: ForgePact issue #14 (crafting at the game's Crafting Cube should use materials held in the shared stash's purchasable material tab). **Phase 0 done 2026-09-22 (`phase0-status: complete`), Phase 1 pending**: the interpretation and the owner's decisions (count and consume; bag first, then the stash tab for the shortfall only; the stash need not be open if its tab is readable closed), the complete static search (97 `craftprobe` rows, including all 29 SDK closures of the cube and stash objects, and the sourced negatives), the vanilla baseline B0 to measure first, hypotheses H-A (count+consume hook), H-B (pull the shortfall stash -> bag through the game's own click-move by name) and H-C (count only), the instrument, the owner-run live procedure, an empty `## Results` table and the owner's `## Decision gate` (`decision: pending`). Nothing ships until the owner picks a mechanism.
...
- `autoprospect bag 1` / `autoprospect bag 0`: Auto-prospect's sub-option (issue #9, Stage C; panel key `mod_auto_prospect_bag`, nested under `mod_auto_prospect` the way `map_reveal_packs` sits under `map_reveal`; **on by default** in the core and the panel, so `build_cmds` emits `autoprospect bag 0` after `autoprospect 1` only when it is off, and turning the parent on live restates it; `autoprospect 1|0` leaves it alone).
  ...
  - `craftprobe hook [substr ...]` / `arm [budget=N] [substr ...]` / `show [all]` / `reset` / `bag|stash|recipe [substr ...]` / `var <Obj|global> <nth> <name> [json]` / `backing on [substr ...]|off|dump|clear` / `dump` / `call <Row> <Obj> <nth> [args ...] confirm` (**research build only**, not in `kPlayerCommands`, dispatched from `HandleCraftCommand`): the issue #14 Phase 1 instrument; see `ForgePact/docs/crafting-materials-research.md` § Instrument. `hook` native-detours all 97 candidate rows (resolved by name, refused outside `Hero_Siege.exe` before `MmCreateHook`), `CheckPlayerInteraction` is the positive control, `arm`/`show` budget and report the per-call self/other/arguments/return lines, the four readers are hook-free, `backing` keeps what the game's own getter calls returned (never invokes one), and `call` is the one write - a single by-name `script_execute` of one plain-script row behind the literal `confirm`, refused before the call on any missing precondition. Several rows share an address with `prospectprobe`'s table: one instrument per session.
