...
23. **"Craft from the stash" (`craftmats`, issue #14) moves the stash's materials and saves the stash at the craft press, a moment the game did not choose - an accepted risk (2026-09-24):**
    ...
    - **What is not observed live** (the owner's choice of two Phase C cases, "Keep two cases"): the Cube fallback (a full bag), a whole-entry take (a shortfall that empties a stash stack), multi-input and multi-unit recipes (the pairing of each count with the decode before it is a static reading, observed only for a one-input recipe), and the game's own hash acceptance of an edited or created item beyond Phase C's watch for a flagged item. The undo shapes a failed take runs (`RemoveItemFromMap` on map 0, `GridRemoveItem` on a bag or Cube grid, `AddItemToMap` on map 9) reuse measured scripts on another map or grid and were not run live.
    ...
    - **Live confirmation:** pending - Phase C (`## Phase C live procedure`) runs on the player DLL `## Ship design` names.
