...
- **The producer can abort the game while it exits (measured from crash dumps, 2026-09-26; not fixed).** `g_publish_worker` is a global `std::thread` that only `StopPublishWorker` joins. When the game leaves through `ExitProcess`, its other threads are gone before the DLL's globals are destroyed, and the still-joinable `std::thread`'s destructor calls `std::terminate`: Windows reports a crash at close (`ucrtbase.dll`, `0xc0000409`, fast-fail 7). 9 of the 10 dumps kept on 2026-09-25/26 show this stack, from the build ForgePact ships (`HSOfflineTrackerProducer.dll`, pinned from ForgePact 1.3.16). A fix joins or detaches the worker before the globals are destroyed, or keeps it where no destructor runs. Evidence: `ForgePact/docs/item-truth-memory-research.md`, "The crash of 2026-09-26 01:36:27", and `RUNTIME_DATA_MODELS.md` §15.
