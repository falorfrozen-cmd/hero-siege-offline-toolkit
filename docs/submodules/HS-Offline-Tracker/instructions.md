...
- **The bundled DLL (recorded 2026-09-19):** `aurie-loader/YYToolkit.dll` is tracked in git, SHA-256 `bb113eefc9a5d485231ced1dc85d773dbc6b762ee680214851c56541359ad297`, 904,192 bytes — the same file ForgePact pins (compared by hash). A modified build of YYToolkit v4.0.1, AGPL-3.0.
...
- **The replacement lives in the hub:** [`third_party/yytoolkit/`](../../../third_party/yytoolkit/README.md) — upstream v4.0.1 (`5a95e46`) plus a documented patch series, built by `tools/build_yytoolkit.py`. Built, host-tested, and **launched once against the game** (2026-09-19, YYToolkit alone, no Tracker producer loaded): the startup fault did not reproduce and lag was not observed in that one session, but no launch has yet loaded the Tracker producer against it, and this is one session on one machine, not a settled fix — see the hub README's "Launch gate" and "First launch results (2026-09-19)" for the full record. This repository still bundles `bb113eef…`.
- **Follow-up (not done), to land together with ForgePact's:** replace the tracked `aurie-loader/YYToolkit.dll` with a launch-gated build of the hub series; rewrite `aurie-loader/yytoolkit-modified/NOTICE.md` to point at the hub directory and delete the two `.cpp` copies; update `THIRD_PARTY_NOTICES.md` and the resources bundled by `src-tauri/tauri.conf.json` so the new notice ships with the binary. Both tools install to `mods/aurie/YYToolkit.dll` and overwrite it — the Game Link installer copies whenever the contents differ, ForgePact copies unconditionally — so a release of only one of them lets the other put the old DLL back.
