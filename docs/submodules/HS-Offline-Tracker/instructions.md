...
- `aurie-loader/`: Bundled Aurie and YYToolkit loader dependencies distributed with the application.
  ...
  - `yytoolkit-modified/`: The AGPL-3.0 notice (`NOTICE.md`) and two whole-file copies — not patches — that accompany the bundled DLL (`Generic-RunnerInterfaceNew.cpp`: runner-interface disk cache and page pre-filter; `Hooks.cpp`: `ExecuteIt` hook left uninstalled). Content-identical to ForgePact's copies apart from line endings. **Incomplete as a source statement for that DLL, and not where YYToolkit is changed** — that is the hub's `third_party/yytoolkit/`.
...
- **The bundled DLL on `origin` (recorded 2026-09-19):** `aurie-loader/YYToolkit.dll` is tracked in git, SHA-256 `bb113eefc9a5d485231ced1dc85d773dbc6b762ee680214851c56541359ad297`, 904,192 bytes — the same file ForgePact pins on its own `origin` (compared by hash). A modified build of YYToolkit v4.0.1, AGPL-3.0.
- **What its notice documents:** `aurie-loader/yytoolkit-modified/NOTICE.md` lists two changes and says everything else is unmodified upstream:
...
- **Open as a pull request against `origin`, not merged, to land together with ForgePact's:** branch `claude/yytoolkit-hs1-distribution` replaces the tracked `aurie-loader/YYToolkit.dll` with the `hs.1` build (sha256 `51a393d7e5291ad76bdb85b9f44faf5178b6b20e0ce8432fa26bdaf9e21eadf8`); rewrites `aurie-loader/yytoolkit-modified/NOTICE.md` to point at the hub directory and deletes the two `.cpp` copies, keeping `YYToolkit-BUILD-INFO.json` beside it; updates `THIRD_PARTY_NOTICES.md`; bundles the notice and BUILD-INFO as resources in `src-tauri/tauri.conf.json`; bumps the version to 0.1.3; adds `scripts/verify-loader.mjs` (wired into `npm run check` as `npm run loader:verify`) so a mismatch between a manifest hash and a file under `aurie-loader/` fails the check by name; and, on an existing installation, classifies the already-installed `YYToolkit.dll` by sha256 as missing / current / superseded / unknown, replacing only a missing or superseded copy and leaving an unrecognised one alone so this installer cannot downgrade a newer loader some other tool put there. Both tools install to `mods/aurie/YYToolkit.dll` and overwrite it — the Game Link installer copies whenever the contents differ, ForgePact copies unconditionally — so a release of only one of them lets the other put the old DLL back; both merge and release together. The launch gate now carries a plugin-loaded row (2026-09-19); what remains is the owner's confirmation, plus gameplay with mods active and the error-report path with a plugin loaded, both still unexercised (`third_party/yytoolkit/README.md`, "Where the binary is published"). The hub release the pin names now exists and is published (not marked `--latest`), and the asset URL it points at resolves — but publishing that release does not merge this pull request, and no Tracker release has shipped it.
...
- Notice for the currently bundled `bb113eef…` DLL (incomplete — see "Bundled Loader & Modified YYToolkit"): `../../../HS-Offline-Tracker/aurie-loader/yytoolkit-modified/NOTICE.md`
