...
const DELTA_SCHEMA = {
  ...
  required: ['exit_code', 'paths', 'instrumentContent', 'sdkContent', 'raw_output'],
}
...
const scribe = (n, what, payload) => agent(
  `You are a scribe. Do exactly this and nothing else, with the Edit tool, in the workorder for '${SLUG}'. ` +
  `Log file: ${A.contextPath} (section '## Log'); State: ${A.planPath} (section '## State'). ${what} ` +
  `Copy every command, output, path:line and quoted string below VERBATIM — never summarise or reword evidence. ` +
...
for (let n = A.round || 0; n < ROUND_CAP; n++) {
  ...
  if (impl.verdict !== 'IMPL-DONE') {
    await scribe(n, `Append a '### Round ${n}' entry recording the implementer's ${impl.verdict} with its evidence, and set State 'round: ${n}', 'phase: blocked'.`, impl)
  ...
  const scope = name => reviewerState[name] === 'never' || !deltaUsable
    ? `Read the whole change: ${diffCommands}`
    : `This is a re-run. Read only these paths changed this round (use the same commands restricted to them): ${delta.paths.join(', ')}`
  ...
  const wrote = await scribe(n,
    `Append a '### Round ${n}' entry: verifier verdict, each failed criterion with its command and real output, each BLOCKING finding with reviewer, where and evidence, the NON-BLOCKING ones as one line each, and 'not re-run: ${skipped.join(', ') || 'none'}'. ` +
    `Then update State: 'round: ${clean || planDefect ? n : n + 1}', 'phase: ${clean ? 'pass' : planDefect ? 'blocked' : 'implement'}', the 'reviewers:' line from reviewerState (append ', not re-run' for: ${skipped.join(', ') || 'none'}), and 'open defects:' (one line each, or 'none').`,
    record)
